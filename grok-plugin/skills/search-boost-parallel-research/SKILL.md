...
Grok Build: use MCP server search-boost from the active user or project config.toml. Native browse remains valid; do not duplicate the same query across both paths. Read the session spill file if a tool result is truncated.
