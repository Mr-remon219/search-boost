...
function pkgBinName() {
  try {
    ...
    return pkg.bin?.['search-boost-mcp'] ? 'search-boost-mcp' : 'search-boost-mcp'
  } catch {
 ...
 * 1. Global `search-boost-mcp` bin (npm i -g)
...
export function resolveMcpLaunch() {
  const bin = pkgBinName()
  if (commandExists(bin)) {
    return { command: bin, args: ['serve'] }
  }

  if (commandExists('npx')) {
    return { command: 'npx', args: ['-y', bin, 'serve'] }
  }
...
const MCP_ENV = {
  SEARCH_BOOST_LAYER: process.env.SEARCH_BOOST_LAYER ?? 'free',
}
