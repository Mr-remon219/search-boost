...
import { PKG_ROOT, getPackageName } from './pkg.mjs'
...
/**
 * Prefer this package's absolute entry: an old global executable may still own
 * the shared `search-boost` bin while migration is in progress.
 */
...
export function formatPrintConfig(agentId, configPath, opts = {}) {
  ...
  if (agentId === 'codex' || agentId === 'grok') {
    if (agentId === 'codex' && opts.replaceNative !== false) {
      lines.push('# Place this setting at the TOML root, before every [table] header.',
        '# If root web_search already exists, replace it rather than adding a duplicate.',
        '# SEARCH_BOOST_WEB_SEARCH_START', CODEX_WEB_SEARCH_BODY, '# SEARCH_BOOST_WEB_SEARCH_END', '')
    }
