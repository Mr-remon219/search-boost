 ...
 *
 * Windows-only: the grok CLI there has been observed to answer "no plugins
 * found in the source" for long plugin source paths and "local path is not a
 * directory" for non-ASCII ones, which reads like a broken bundle rather than a
 * path the CLI cannot take. The same CLI version accepts both on Linux, so
 * nothing is skipped or pre-checked up front and no length threshold is encoded
 * here — this only explains a failure that already happened on Windows.
 *
...
export function grokInstallFailureHint(pluginDir, platform = process.platform) {
  ...
  return [
    ...
    'The Windows grok CLI can reject long or non-ASCII sources this way; copy grok-plugin to a short ASCII path (e.g. C:\\sb\\grok-plugin) and run the command above against that copy.',
  ].join('\n  ')
}
...
export function installGrokPlugin(opts = {}) {
  ...
  if (result.status !== 0) {
    warnInstallFailure(`exit ${result.status}`, cmdLine, pluginDir)
    return { ok: true, exitCode: result.status ?? null }
  }
  return { ok: true }
}
