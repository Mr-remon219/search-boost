 ...
 * Returns null only when a valid plugin listing confirms absence.
...
export async function resolveInstalledGrokPluginId() {
  ...
  if (!Array.isArray(plugins)) {
    if (!Array.isArray(plugins?.plugins)) return GROK_PLUGIN_NAME
    plugins = plugins.plugins
  }
  if (plugins.some((p) => !p || typeof p !== 'object' || Array.isArray(p))) return GROK_PLUGIN_NAME
