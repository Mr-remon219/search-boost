...
export async function runUpgrade({ dryRun = false, syncOnly = false, workspace, run = runCommand, log = console.log } = {}) {
  ...
  try {
    if (!syncOnly) {
      ...
      let raw
      try { raw = await checkedCommand(run, 'npm', ['view', `${PACKAGE}@latest`, 'version', '--json'], { timeoutMs: 30_000 }) }
      catch { throw new Error('npm version check failed: search-boost@latest may not be published yet, or the registry/network is unavailable. No package or integration was changed; retry after the new release is available.') }
