...
export async function runUpgrade({ dryRun = false, syncOnly = false, workspace, run = runCommand, log = console.log } = {}) {
  ...
  try {
    ...
    // Runtime migrations share package-source receipts/rollback state; serialize
    // that lane. Independent MCP hosts and Grok run alongside it, never behind
    // a hung DSH/Pi command. Keep deterministic result order despite concurrency.
    const jobs = plan.targets.map((target) => ({
      lane: target.kind === 'pi' || target.kind === 'dsh' ? 'runtime-receipts'
        : target.id === 'grok' ? 'grok' : target.paths?.mcp ?? target.paths?.config ?? target.label,
      run: async () => {
    ...
    jobs.forEach((job, index) => {
      if (!lanes.has(job.lane)) lanes.set(job.lane, [])
      lanes.get(job.lane).push({ ...job, index })
    })
    await pool([...lanes.values()], 3, async (lane) => {
