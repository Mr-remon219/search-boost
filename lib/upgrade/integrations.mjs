import { migrateCodexNativeSearch } from '../codex-native.mjs'
...
import { packageIdentity, inspectPackageSource, splitSourceModifier, localPackagePath, PI_PACKAGE_NAMES, DSH_PACKAGE_NAMES } from '../package-identity.mjs'
import { PKG_ROOT } from '../pkg.mjs'
import { packageSourcesPath, recordPackageSources } from '../package-sources.mjs'
...
import { fileResource } from './scheduler.mjs'
    ...
    await inspect(`pi (${agentDir})`, async () => {
      ...
      for (const name of ['search-boost', LEGACY_PI]) {
        ...
        for (const file of ['index.ts', 'index.js']) {
          if (PI_PACKAGE_NAMES.includes(await packageIdentity(join(dir, file), agentDir))) { legacyDirs.push(dir); break }
        }
      }
...
/** One source of truth for both MCP rollback and scheduling conflicts. */
function mcpFiles({ id, paths: p }) {
  return [...new Set([p.mcp ?? p.config, ...skillFiles(id, p.skill), p.hooks, p.hookScript, p.hookInject, p.agents, p.gemini, p.rule].filter(Boolean))]
}

export function integrationResources(target) {
  const resources = target.kind === 'mcp'
    ? mcpFiles(target).map(fileResource) : ['runtime-receipts']
  // All project receipts use the same read-modify-write store, including Pi.
  if (target.root || target.kind === 'pi') resources.push('upgrade-projects')
  // The native plugin manager can touch Grok host files as well.
  if (target.id === 'grok') resources.push('grok')
  return resources
}
...
async function refreshMcp(target, dryRun) {
  ...
  let toml
  if (id === 'codex' || id === 'grok') {
    let previous = await optionalText(config) ?? ''
    if (id === 'codex') previous = migrateCodexNativeSearch(previous)
    toml = refreshTomlMcp(previous, launch)
  }
...
function assertNotDowngrade(installed, replacement) {
  if (installed?.name === PACKAGE && typeof installed.version === 'string' && compareVersions(installed.version, replacement.version) > 0) {
    throw new Error(`Installed ${PACKAGE} ${installed.version} is newer than running ${replacement.version}; refusing downgrade`)
  }
}

async function refreshPi(target, dryRun, run, replacement) {
  const { agentDir, settingsPath, shim, legacyDirs } = target
  const originalSettings = await strictJson(settingsPath)
  let packaged = false
  const adapter = join(replacement.root, 'adapters', 'pi', 'index.js')
  const childMigration = migratePiSubagentSettings(originalSettings, agentDir, adapter)
  const settings = childMigration.settings
  for (const dir of legacyDirs) assertNotDowngrade(inspectPackageSource(dir, agentDir), replacement)
  for (const field of ['packages', 'extensions']) {
    if (!settings[field]) continue
    if (!Array.isArray(settings[field])) throw new Error(`Invalid ${field} in ${settingsPath}`)
    const next = []
    for (const entry of settings[field]) {
      const installed = inspectPackageSource(sourceOf(entry), agentDir)
      const identity = installed?.name
      if (!PI_PACKAGE_NAMES.includes(identity)) { next.push(entry); continue }
      assertNotDowngrade(installed, replacement)
      packaged = true
      const { modifier } = splitSourceModifier(sourceOf(entry))
      const source = modifier + (field === 'packages' ? replacement.root : adapter)
  ...
  const pairs = [...piSubagentTemplatePaths().map((src) => ({ src: join(replacement.root, relative(PKG_ROOT, src)), dest: join(agentDir, 'agents', src.split(/[\\/]/).at(-1)) })), ...piWorkflowPromptPaths().map((src) => ({ src: join(replacement.root, relative(PKG_ROOT, src)), dest: join(agentDir, 'prompts', src.split(/[\\/]/).at(-1)) }))]
  for (const { src, dest } of pairs) {
    if (!(await optionalText(src))?.includes(PI_OWNED_MARKER)) throw new Error(`Replacement Pi workflow is missing its ownership marker: ${src}`)
  ...
  if (priorShim?.includes(PI_SHIM_MARKER)) assertNotDowngrade(inspectPackageSource(shim, agentDir), replacement)
  if (!packaged && !priorShim && !legacyDirs.some(existsSync)) throw new Error('Pi registration disappeared; refusing a new installation during upgrade')
  if (priorShim && !priorShim.includes(PI_SHIM_MARKER)) throw new Error(`User-owned Pi shim at ${shim}; left unchanged`)
  if (dryRun) return null
  const backup = await transactional([settingsPath, shim, packageSourcesPath(), ...pairs.map((p) => p.dest)], async (backup) => {
    const moved = []
    try {
      for (const { src, dest } of pairs) {
      ...
      if (packaged || childMigration.changes.length) {
      ...
      if (packaged) {
        ...
        await writeFile(shim, `// ${PI_SHIM_MARKER}\nexport { default } from ${JSON.stringify(pathToFileURL(adapter).href)}\n`)
      }
      recordPackageSources(agentDir, [replacement.root, adapter, shim], replacement)
      // Verify what a subsequent invocation will discover, before archiving legacy files.
      if (packaged || childMigration.changes.length) {
        const written = await strictJson(settingsPath)
        for (const field of ['packages', 'extensions', 'subagents']) {
          if (JSON.stringify(written[field]) !== JSON.stringify(settings[field])) throw new Error('Pi registration verification failed')
        }
      } else if (inspectPackageSource(shim, agentDir)?.root !== replacement.root) throw new Error('Pi shim did not resolve to the replacement')
...
/** Check both the persisted next-boot source and the currently resolvable payload. */
async function verifyDshRegistration(dir, pkg, replacement) {
  const spec = pkg.dependencies?.[PACKAGE] ?? pkg.devDependencies?.[PACKAGE]
  if (typeof spec !== 'string') throw new Error('DSH did not register the new bundle; keeping the old registration')
  const installedSource = spec.startsWith('link:') || spec.startsWith('file:')
    ? resolve(dir, spec.slice(spec.indexOf(':') + 1)) : localPackagePath(spec, dir)
  if (installedSource !== replacement.root) throw new Error('DSH did not switch the dependency source to the replacement')
  await verifyReplacement(join(dir, 'node_modules', PACKAGE), replacement.version, { linkedTo: replacement.root })
}

async function refreshDsh(target, dryRun, run, replacement) {
  const installedRoot = join(target.dir, 'node_modules', PACKAGE)
  assertNotDowngrade(inspectPackageSource(installedRoot, target.dir), replacement)
  const files = ['package.json', 'pnpm-lock.yaml', 'cordis.yml', 'cordis.yaml', 'cordis.patch.yml'].map((f) => join(target.dir, f))
  if (!existsSync(files[0])) throw new Error('DSH profile disappeared; refusing to create it during upgrade')
  const original = await strictJson(files[0])
  const originalBundles = original.dsh?.profile?.bundles ?? []
  if (!Array.isArray(originalBundles) || originalBundles.some((name) => typeof name !== 'string')) throw new Error('Invalid DSH bundle list; left unchanged')
  if (!DSH_PACKAGE_NAMES.some((name) => original.dependencies?.[name] || original.devDependencies?.[name] || originalBundles.includes(name))) throw new Error('DSH registration disappeared; refusing a new installation during upgrade')
  ...
  if (dryRun) return null
  const backup = await transactional([...files, packageSourcesPath()], async () => {
    await checkedCommand(run, 'dsh', ['plugin', '--profile', target.profile, 'add', replacement.root])
    const pkg = await strictJson(files[0])
    await verifyDshRegistration(target.dir, pkg, replacement)
    ...
    recordPackageSources(target.dir, [replacement.root, installedRoot], replacement)
  })
  try {
...
export async function refreshIntegration(target, { dryRun = false, run, packageRoot = PKG_ROOT } = {}) {
  if (target.kind === 'mcp') return refreshMcp(target, dryRun)
  const root = realpathSync(resolve(packageRoot))
  const { version } = await strictJson(join(root, 'package.json'))
  await verifyReplacement(root, version)
  const replacement = { root, version }
  if (target.kind === 'pi') return refreshPi(target, dryRun, run, replacement)
  if (target.kind === 'dsh') return refreshDsh(target, dryRun, run, replacement)
...
export async function refreshGrokPlugin({ dryRun, run }) {
  let result
  try { result = await run('grok', ['plugin', 'list', '--json'], { timeoutMs: 15_000 }) } catch { return { status: 'unavailable', message: 'Grok CLI unavailable; plugin installation could not be inspected.' } }
  ...
  await checkedCommand(run, 'grok', ['plugin', 'install', join(PKG_ROOT, 'grok-plugin')], { timeoutMs: 60_000 })
  const verifiedRaw = await checkedCommand(run, 'grok', ['plugin', 'list', '--json'], { timeoutMs: 15_000 })
  let verified
