...
import { setCodexNativeSearch, codexRootSearchDisabled } from './codex-native.mjs'
import {
  jsonDeepEqual,
  prunePermissions,
  readJsonFile,
...
export function claudePriorInstallArtifacts(homeDir = homedir()) {
  ...
  return {
    ...
    allowConfigured: allow.some((p) => typeof p === 'string' && p.startsWith('mcp__search-boost__')),
...
export function migrateLegacyClaudeNativeDeny(settings, hasMcp) {
  ...
  if (!deny.includes(CLAUDE_WEB_SEARCH_DENY)) return settings
  const allow = settings.permissions?.allow ?? []
  if (!allow.some((p) => typeof p === 'string' && p.startsWith('mcp__search-boost__'))) return settings
...
export function applyCodexNativeToml(toml, replace) {
  return setCodexNativeSearch(toml, replace)
}

/** @param {string} toml */
export function codexNativeReplaced(toml) {
  return codexRootSearchDisabled(toml)
}
...
export async function applyNativeSearch(id, opts) {
  ...
  if (kind === 'config' && id === 'codex') {
    ...
    if (!opts.dryRun) {
      if (next !== toml) await writeTextFile(file, next)
    }
    return [file]
  }
