...
function assertResearchTools(tools, agent) {
  if (typeof tools?.get !== 'function') throw new Error('research_parallel: DSH tool registry unavailable; cannot verify searcher tools')
  const missing = RESEARCH_TOOLS.filter((name) => !tools.get(name, agent))
  if (missing.length) throw new Error(`research_parallel: required searcher tools unavailable in this DSH scope: ${missing.join(', ')}; check bundle registration, fusedSearch/fetchPage flags and host tool restrictions (no permission widening)`)
}
...
export async function parallelResearch(opts) {
  ...
  if (!opts.agent) throw new Error('research_parallel: a live DSH parent agent is required')
  // Provider capability flags and tool allowlists do not prove registration.
  // Use the caller's scoped view, never the unscoped/global registry.
  if (!opts.signal?.aborted && tasks.some((item) => item.agent === 'searcher')) assertResearchTools(opts.tools, opts.agent)
  ...
  async function attempt(item) {
    ...
    try {
      ...
      observed.catch(() => {})
      // Native in-process providers expose the actual child. Check that scope as
      // well: a provider may add restrictions beyond the parent's preset.
      if (!controller.signal.aborted && item.agent === 'searcher' && run.localAgent) assertResearchTools(opts.tools, run.localAgent)
