...
export function proxyPolicy(env = process.env) {
  ...
  if (!chosen) return { mode: 'direct', url: null, source: null, noProxy: env.no_proxy ?? env.NO_PROXY ?? null }
  let protocol
  try {
    protocol = new URL(chosen.value).protocol.replace(':', '')
  } catch {
    return {
      mode: 'proxy',
      url: chosen.value,
      source: chosen.name,
      protocol: null,
      error: new NetworkPolicyError(`proxy variable ${chosen.name} is not a valid URL`, NET_ERROR_KINDS.proxyConfig, { detail: 'invalid_url' }),
      noProxy: env.no_proxy ?? env.NO_PROXY ?? null,
    }
  }
  if (protocol !== 'http' && protocol !== 'https') {
    // SOCKS is not supported by the locked Undici release: fail explicitly
    ...
    return {
      ...
      source: chosen.name,
      protocol,
      error: new NetworkPolicyError(
        `proxy ${chosen.name}=${protocol}:// is not supported; use an HTTP/mixed proxy port (Undici ${protocol}:// support is unavailable)`,
  ...
  return { mode: 'proxy', url: chosen.value, source: chosen.name, protocol, noProxy: env.no_proxy ?? env.NO_PROXY ?? null }
}
