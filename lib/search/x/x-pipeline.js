...
export function normalizePosts(raw) {
  return asArray(raw).flatMap((p) => {
    ...
    if (p.engineRanks && typeof p.engineRanks === 'object') out.engineRanks = Object.fromEntries(Object.entries(p.engineRanks).filter(([e, r]) => out.engines?.includes(e) && Number.isSafeInteger(r) && r >= 1))
    if (Array.isArray(p.provenance)) out.provenance = p.provenance.map((entry) => ({ ...entry }))
...
function dedupe(rows, userMode) {
  return dedupeBy(rows, (row) => userMode ? row.item.username : row.item.id, (prior, row) => {
    for (const [k, v] of Object.entries(row.item)) {
      if (k === 'engineRanks' || k === 'provenance' || (userMode && k === 'recent_posts')) continue
    ...
    if (row.item.engineRanks) {
      prior.item.engineRanks = { ...prior.item.engineRanks }
      for (const [engine, rank] of Object.entries(row.item.engineRanks)) prior.item.engineRanks[engine] = Math.min(prior.item.engineRanks[engine] ?? Infinity, rank)
    }
    if (row.item.provenance) prior.item.provenance = [...(prior.item.provenance ?? []), ...row.item.provenance]
...
export function createXPipeline(args, limit = 5) {
  ...
  return {
    ...
    finish(batches, { applyConstraints = true, limitResults = true } = {}) {
      ...
      // Attach ranks BEFORE normalization/filtering/dedupe; never use merged rank
      // as if each contributing provider returned the same original position.
      const rows = dedupe(batches.flatMap(({ source, data }) => {
        const ranked = asArray(data).map((item, i) => {
          if (!item || typeof item !== 'object') return item
          if (source === 'official') return { ...item, engines: ['x-official'], engineRanks: { 'x-official': i + 1 }, provenance: undefined }
          if (source === 'engines' && !item.engines?.length) return { ...item, engines: ['x-fallback'], engineRanks: { 'x-fallback': i + 1 } }
          return item
        })
        return (userMode ? normalizeUsers(ranked) : normalizePosts(ranked)).map((item) => ({ source, item }))
      }), userMode)
