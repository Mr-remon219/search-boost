...
/**
 * Write a JSON file atomically and privately (0600 via a unique exclusive temp
 * file + rename). `tightenDir` is only enabled for directories we own: grok's
 * own ~/.grok directory is not ours to chmod.
 * @param {string} file
 * @param {unknown} value
  ...
  try {
    ...
    for (const key of Object.keys(parsed)) {
      ...
      if (entry && typeof entry === 'object' && typeof entry.key === 'string') {
        ...
        // Grok's own directory is not ours to tighten; the file itself is 0600.
        writeJsonAtomic(file, parsed, { tightenDir: false })
        return
      }
    }
  } catch { /* best-effort */ }
 ...
 */
export function importFromGrok({ dryRun = false } = {}) {
  const grok = readGrokAuth()
  if (!grok) {
    throw new Error(`no grok login found at ${grokAuthFile()} — run \`grok login\` first (or ${XAUTH_KEY_HINT})`)
  }
  const entry = {
    kind: 'grok-session',
    key: grok.key,
  ...
  if (!dryRun) savePiAuth(entry)
  return entry
}

/** `/x-login -k <key>`: store an API key (public api.x.ai path). */
export function importApiKey(key, { dryRun = false } = {}) {
...
/** True when a stored auth entry can be used synchronously (key present, not expired). */
export function isAuthEntryUsable(entry) {
  if (!entry?.key) return false
  if (entry.kind === 'api-key') return true
  if (entry.expires_at) {
    const ms = Date.parse(entry.expires_at)
 ...
 * /x-login api-key, grok-session, and the credential-free fallback path so
 * official and fallback results never cross in the cache.
 */
export function xAuthCacheToken() {
  ...
  if (entry.kind === 'api-key') return `local-key:${entry.key.slice(0, 12)}`
  return `local-grok:${entry.key.slice(-20)}`
}

export const XAUTH_IMPORT_HINT = 'search-boost config x --import-grok (or /x-login in MCP)'
export const XAUTH_KEY_HINT = 'search-boost config x --set-xai-key <KEY> (or /x-login -k in MCP)'
...
export function authStatus() {
  ...
  if (grok) {
    ...
    return {
      source: 'grok-pending',
      detail: `${grokAuthFile()} (${grok.email ?? grok.user_id ?? '?'}, tier=${tierName(claims?.tier)})${expDetail(grok)} — not imported`,
    }
  }
  return { source: 'none', detail: `no credentials — run ${XAUTH_IMPORT_HINT} or set XAI_API_KEY` }
}
...
export async function refreshOidcToken(entry, signal) {
  ...
  let res
  try {
    res = await ipv4Fetch(tokenEndpoint, {
      method: 'POST',
