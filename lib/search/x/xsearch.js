...
// structured JSON template (posts / user / thread) that we salvage.
// The runtime normalizes and filters all providers through x-pipeline.js. When the official path is unavailable or rejected, callers
...
import { xIdentity } from './x-pipeline.js'
export { normalizePosts } from './x-pipeline.js'
...
const POST_SCHEMA = '{"id":str,"author":str,"username":str,"text":str,"url":str,"likes":num,"reposts":num,"replies":num,"views":num,"created_at":str,"lang":str,"media":[str]}'
...
export function buildXSearchPrompt(kind, params, maxResults) {
  ...
  switch (kind) {
    ...
    case 'user':
      ...
      break
    case 'thread': {
      const id = xIdentity(params.post_id).id || params.post_id
  ...
  return [
    ...
    task,
    params.username && !isUser ? `Only consider posts authored by @${params.username}.` : '',
    ...
    isUser ? `Each recent_posts item uses this post schema: ${POST_SCHEMA}` : POST_SCHEMA,
    'Include created_at (ISO8601) and lang when known. Omit unknown fields, especially dates and engagement counts; never substitute zero.',
...
export async function runXTool(params, signal) {
  ...
  const postOnce = async (authHeaders) => {
    let res
    try {
      res = await ipv4Fetch(`${baseUrl}/responses`, {
        method: 'POST',
  ...
  // Provider decoding only. Runtime applies the shared result pipeline once,
  // after combining hosted and credential-free candidates.
  const data = (Array.isArray(parsed) ? parsed : [parsed]).filter((item) => item && typeof item === 'object')
  if (data.length === 0) {
