...
async function guestToken(signal) {
  ...
  try {
    writeJsonAtomic(prepareConfigWrite('xguest'), { token: d.guest_token, ts: Date.now() })
...
function tweetToPost(t) {
  ...
  const username = String(userCore.screen_name ?? '')
  const id = String(inner.rest_id ?? inner.id_str ?? legacy.id_str ?? '')
  const text = String(inner.note_tweet?.note_tweet_results?.result?.text ?? legacy.full_text ?? legacy.text ?? '')
  if (!id && !text) return null
  const views = inner.views?.count
  const media = (legacy.extended_entities?.media ?? [])
    .map((m) => m.media_url_https ?? m.url)
    .filter(Boolean)
  return {
    id,
    author: userCore.name ?? username,
    username,
    text,
...
export async function guestUser(username, limit = 3, signal) {
  ...
  try {
    const d2 = await graphqlGet(
      ...
      {
        userId: user.id,
        count: Math.min(Math.max(limit ?? 3, 1) * 3, 100),
    ...
    user.recent_posts = parseTweets(d2) // final constraints/limit belong to Core
...
export async function oembedPost(postId, signal) {
  const id = xIdentity(postId).id || (/^\d+$/.test(String(postId)) ? String(postId) : '')
  ...
  const handle = xIdentity(d.author_url).username
  return {
    id,
...
export function hitToPost(h) {
  ...
  return {
    id: xIdentity(url).id ?? '',
    engines: h.engines,
    engineRanks: h.engineRanks,
    created_at: h.created_at ?? h.published,
    author: author ?? undefined,
    username: xIdentity(url).username,
    text: text || (h.snippet ?? ''),
    url,
  }
}
...
export async function fallbackXSearch({ type, query, username, post_id, limit = 3, webSearch, signal }) {
  const max = Math.min(Math.max(limit ?? 3, 1), 30)

  // ---- thread: oEmbed (single post; engines cannot rebuild a thread tree) ----
  if (type === 'thread') {
    const id = xIdentity(post_id).id || (/^\d+$/.test(String(post_id)) ? String(post_id) : '')
  ...
  if (type === 'user') {
    ...
    if (webSearch) {
      try {
        ...
        const profiles = hits
          .filter((h) => { const identity = xIdentity(h.url); return identity.username && !identity.id })
          .map((h) => ({
            id: '',
            name: h.title ?? '',
            username: xIdentity(h.url).username,
            bio: '',
            recent_posts: [],
            url: h.url ?? '',
          }))
  ...
  if (webSearch) {
    try {
      ...
      const posts = hits.filter((h) => xIdentity(h.url).id).map(hitToPost)
      if (posts.length) {
