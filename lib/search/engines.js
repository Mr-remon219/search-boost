...
export const ENGINE_ORDER = ['bing', 'ddg', 'yahoo', 'exa-free', 'tavily', 'brave', 'exa', 'anysearch']
...
// Never read, log, persist, or retry an error body (including business errors).
async function anysearchSearch(query, count, opts = {}, keys) {
  ...
  return json.data.results.filter((r) => r && typeof r.url === 'string').slice(0, count).map((r) => ({
    title: collapseSpace(r.title), url: r.url,
