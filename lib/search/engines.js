...
async function yahooSearch(query, count, opts) {
  ...
  const hits = []
  const seen = new Set()
  const push = (href, titleHtml, snippetHtml = '', providerRank) => {
  ...
  for (const [index, block] of blocks.entries()) {
    ...
    push(anchor[1], titleM[1], snippetM?.[1] ?? '', index + 1)
  }
  // fallback: any compTitle + title h3 pair the block split missed
  if (hits.length < count) {
    // Supplemental parse positions follow the primary stream, including slots
    // whose URL/title was invalid. Neither filtering nor dedup promotes a hit.
    let fallbackRank = blocks.length
    for (const m of html.matchAll(/<div class="compTitle[^"]*"[\s\S]*?<a[^>]+href="([^"]+)"[\s\S]*?<h3[^>]*class="[^"]*title[^"]*"[^>]*>([\s\S]*?)<\/h3>/gi)) {
      push(m[1], m[2], '', ++fallbackRank)
      if (hits.length >= count) break
    }
  }
...
async function anysearchSearch(query, count, opts = {}, keys) {
  const headers = { 'content-type': 'application/json' }
  if (opts.enginePool !== 'free' && keys.anysearch) headers.authorization = `Bearer ${keys.anysearch}`
  const res = await ipv4Fetch('https://api.anysearch.com/v1/search', {
    method: 'POST', headers,
...
export function engineRegistry(keys, enabledKeyed = null) {
  ...
  return {
    anysearch: {
      ...
      search: (q, n, o) => anysearchSearch(q, n, o, keys),
    },
    bing: {
      available: () => true,
      search: bingSearch,
