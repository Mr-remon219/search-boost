...
function answerCapableOf(assoc, thresholds) {
  ...
  // being read as a confident negative.
  const judgmentIncomplete = judgment.relevant === null || judgment.states_evidence === null || judgment.injection === null
  ...
  return { assessed: true, judgmentIncomplete, injectionSuspected, relevant, statesEvidence, premiseConflict, answerCapable: relevant && statesEvidence && !injectionSuspected }
}
...
export async function runAdaptiveLoop(input, deps = {}) {
  ...
  if (candidateSet.size === 0) {
    return earlyResult({
      ...
      inputQuestions: rawQuestions,
      warnings: ['No available engines for the active layer. Configure keys (`search-boost config keys`) or switch the layer (`/web_change free`), then retry.'],
      startedAt,
      now,
      jevState,
      limits,
  ...
  function reserve(kind, count = 1) {
    ...
    if (kind === 'jev') {
      ...
      if (used.estimatedTokens >= budget.maxJevInputTokens) return false
      used.jevCalls += count
      return true
    }    return false
  }
  ...
  async function jevAsk(phase, request) {
    ...
    if (!reserve('jev')) {
      const tokens = used.jevInputTokens >= budget.maxJevInputTokens || used.estimatedTokens >= budget.maxJevInputTokens
      throw new BudgetExhausted(tokens ? STOP_REASONS.budgetTokens : STOP_REASONS.budgetCalls)
    }
    used.estimatedTokens += estimate
  ...
  function selectEnginesFromEntries(entries, offered, threshold, questionId) {
    const valid = []
    let invalid = 0
    ...
    const offeredEngines = offered.filter((item) => item.questionId === questionId).map((item) => item.engine)
    if (!valid.length) {
  ...
  async function planEngineRound(unfinished, roundEntry) {
    ...
    for (const q of canonical) {
      ...
      const offered = request.offered.filter((item) => item.questionId === q.id && !alreadyRan[q.id].includes(item.engine))
      if (!offered.length) continue
      const { engines: chosen, mode, invalid } = selectEnginesFromEntries(result.entries, offered, thresholds.engineSelect, q.id)
  ...
  function feasibleActions(questionId) {
    ...
    if (fetchBudgetLeft) {
      ...
      for (const item of ranked) {
        ...
        if (source.fetch?.state === 'ok') continue
  ...
  async function planActionRound(unfinished, roundEntry) {
    ...
    if (actionsByQuestion.length) {
      ...
      if (engineQuestions.length) {
        ...
        request.state.engines = engineRequest.state.engines
      ...
      for (const entry of actionsByQuestion) {
        ...
        if (action.key === 'search_new_engines') {
          const offered = engineChoices?.offered.filter((item) => item.questionId === entry.questionId) ?? []
          const chosen = selectEnginesFromEntries(result.entries, offered, thresholds.engineSelect, entry.questionId)
          const engines = chosen.engines.filter((name) => candidateSet.has(name))
          selections.push({ questionId: entry.questionId, action: { ...action, params: { ...action.params, engines } }, mode, engineMode: chosen.mode })
        } else {
  ...
  async function executeSearch({ questionId, engines, complexity, mode }) {
    ...
    const selected = engines.filter((name) => live.has(name) && candidateSet.has(name))
    const dropped = engines.filter((name) => !selected.includes(name))
  ...
  async function executeFetch({ questionId, url, sourceKey }) {
    ...
    if (!reserve('fetchRead')) return { questionId, url, skipped: true, reason: REASONS.budgetExhausted }
    executed.add(signature)
    const question = canonical.find((q) => q.id === questionId)
    const started = now()
    let reservedNetwork = reserve('fetch')
    try {
      let page = await deps.runFetchPage(url, question.text, signal)
      if (page?.cacheHit && reservedNetwork) {
        used.fetchCalls-- // refund: a cache read is not a new network fetch
        reservedNetwork = false
      }
      if (page?.focusMiss && used.fetchReads < budget.maxFetchReads) {
        // A focus miss does not mean the page has no answer: re-read the same
        // cached page without focus inside the remaining budget. A cache hit
        // costs no network fetch.
        used.fetchReads++
        const reread = await deps.runFetchPage(url, undefined, signal)
        if (reread?.cacheHit && reservedNetwork) {
          used.fetchCalls--
          reservedNetwork = false
        }
        if (reread?.content) page = { ...reread, focusMiss: true }
      }
  ...
  async function coverageJudgeRound(roundEntry) {
    ...
    for (const chunk of chunks) {
      ...
      for (const item of request.mapping) {
        ...
        const conflicts = []
        if (conflictRead.valid && conflictRead.value !== null && conflictRead.value > thresholds.sourceConflict) {
        ...
        } else if (!hasStrongBasis && !(snippetRead.valid && snippetRead.value !== null && snippetRead.value > thresholds.snippetSelfSufficient)) {
  ...
  async function fallbackSearchRound(reason) {
    ...
    const tasks = canonical.map((q) => async () => {
  ...
  try {
    while (true) {
      ...
      used.rounds++
      const roundEntry = { round: used.rounds, plan: null, search: [], fetch: [], judgment: null }
      ...
      // ---- EXECUTE ----
      const tasks = plan.map((selection) => async () => {
  ...
  function buildEvidenceView(classified, coverageIds) {
    ...
    for (const item of classified) {
      ...
      items.push({
        ...
        fetch: source?.fetch ? { state: source.fetch.state, via: source.fetch.via ?? null, words: source.fetch.words ?? 0 } : null,
      })
    }
  ...
  function assembleResult(final) {
    ...
    const result = {
      ...
      evidence: {
        ...
        answerCapable: evidence.stats().answerCapable,
