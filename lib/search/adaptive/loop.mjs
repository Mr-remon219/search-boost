...
import { queryCandidates, addQueryChoices } from './planning.js'
...
import { coverageRequest, explicitTokens, planActionRequest, planEngineRequest, sourceJudgeRequests, textStatesToken } from './prompts.js'
...
export async function runAdaptiveLoop(input, deps = {}) {
  ...
  const validated = normalizeAdaptiveInput(input, limits)
  if (validated.error) {
    return earlyResult({
  ...
  const canonical = canonicalTargets(validated.targets).map((q) => ({ ...q, timeWindow: q.timeRange ?? dateWindow(q.acceptance ?? q.text, startedAt) }))
  ...
  const progress = new Map(canonical.map((q) => [q.id, { engines: new Set(), successful: new Set(), complexity: null, searched: 0, history: [] }]))
  ...
  async function jevAsk(phase, request) {
    ...
    const phaseKey = `${used.rounds}:${phase}`
  ...
  function feasibleActions(questionId) {
    ...
    if (searchBudgetLeft && deepenEngines.length) {
      options.push({
        key: 'deepen',
    ...
    if (searchBudgetLeft && variants.length && state.searched > 0) {
  ...
  async function executeSearch({ questionId, engines, complexity, mode, query }) {
    ...
    try {
      ...
      const ingest = evidence.ingestSearch({ questionId, round: used.rounds, results: result?.results ?? [], query, engines: selected, complexity })
  ...
  async function executeFetch({ questionId, url, sourceKey }) {
    ...
    const started = now()
    try {
      const first = await readPage(url, question.text, () => executed.add(signature))
      if (first.skipped) {
  ...
  function selectPendingCandidates(pending) {
    ...
    const out = []
    const rotation = (used.rounds - 1) % Math.max(1, groups.length)
  ...
  async function sourceJudgeRound(roundEntry) {
    const pending = evidence.pendingSourceJudge().filter((item) => !finished.has(item.questionId))
    ...
    const scheduled = requests[0]?.mapping.length ?? 0
    if (pending.length > scheduled) roundEntry.deferredSourceJudgments = pending.length - scheduled
    let judged = 0
    ...
    sourceJudgeRuns++
    roundEntry.judgment = { ...(roundEntry.judgment ?? {}), sourceJudge: { candidates: scheduled, judged } }
    return { judged }
  }
  ...
  function qualifiedFor(questionId) {
    ...
    const qualified = answerCapable.map((item) => {
      ...
      return {
        ...
        sourceId: source?.id,
        quality: Math.min(item.assoc.judgment?.relevant ?? 0, item.assoc.judgment?.states_evidence ?? 0),
  ...
  async function coverageJudgeRound(roundEntry) {
    ...
    for (const entry of ordered) {
      if (requestFits(coverageRequest({ questions: [...current, entry], limits }), limits)) current.push(entry)
    }
    const chunks = current.length ? [current] : []
    if (current.length < entries.length) roundEntry.deferredCoverageJudgments = entries.length - current.length
  ...
  async function fallbackSearchRound(reason) {
    ...
    await runContained(canonical, async (q) => {
      const engines = defaultEngineSubset([...candidateSet], Math.min(3, candidateSet.size))
