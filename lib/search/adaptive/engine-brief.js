...
const STATIC_TRAITS = {
  anysearch: { kind: 'aggregator', cost: 'pool-dependent', text: 'snippet and optional cleaned content', traits: ['one logical provider; internal source independence is unknown', 'free pool uses anonymous IP quota; api requires a key; hybrid uses key when configured', 'up to 10 results; site: hint only, no native date filter'] },
  bing: {
    kind: 'html-index',
    cost: 'free',
  ...
  ddg: {
    ...
    text: 'title and short snippet only',
    traits: [
      'keyless HTML scrape, separate retrieval endpoint; results can overlap Bing',
  ...
  yahoo: {
    ...
    text: 'title and short snippet only',
    traits: [
      'keyless HTML scrape, another retrieval endpoint; not proof of source independence',
      'no server-side host filter; a site: hint is folded into the query text',
      'no date filter and no publication date returned',
      'returns no page body',
    ],
...
export const ENGINE_NAMES = ['bing', 'ddg', 'yahoo', 'exa-free', 'tavily', 'brave', 'exa', 'anysearch']
