...
/** Rendered state entry for one canonical question. */
function questionState(questions, index) {
 ...
 * @param {{ questions: any[], engines: any[], alreadyRan: Record<string, string[]>, round: number, limits?: typeof ADAPTIVE_LIMITS }} input
 */
export function planEngineRequest({ questions, engines, alreadyRan = {}, round = 1, limits = ADAPTIVE_LIMITS }) {
  const state = {
    ...
    constraints: [
      ...
      'Only entries with available=true in state.engines may run.',
      `cost=paid consumes the user's own API quota; prefer free engines when they can answer.`,
      'Engine/question pairs listed in state.already_ran already executed for that question and are not re-run with the same parameters.',
      'Availability is configuration readiness, not a connectivity guarantee.',
    ],
    questions: questions.map((_, index) => questionState(questions, index)),
    engines,
  ...
  questions.forEach((q, qi) => {
    engines.forEach((engine, ei) => {
      ...
      questionMap[id] = noul(
        `Does state.engines[${ei}] (engine "${engine.name}", kind ${engine.kind}, cost ${engine.cost}, returns ${engine.returns_text}) belong to the engines that should run for the question in state.questions[${qi}].text (you may also read state.already_ran[${qi}].engines, which lists engines already executed for it)?`,
        {
          true: `state.engines[${ei}] can plausibly add evidence for state.questions[${qi}].text and is not already covered by state.already_ran[${qi}].engines`,
          false: `state.engines[${ei}] is unlikely to add distinct evidence for state.questions[${qi}].text`,
        },
      )
