...
import { excerptForTool, pickParagraphs, isBoilerplate } from '../evidence.js'
...
export function excerptForContent(content, questionText, limits = ADAPTIVE_LIMITS) {
  ...
  const clean = String(content).split(/\n{2,}/).filter((block) => !isBoilerplate(block)).join('\n\n')
  if (!isMaterialText(clean, limits)) return { text: '', basis: null }
  const picked = pickParagraphs(clean, questionText, limits.maxExcerptsPerEvidence, limits.maxExcerptChars)
  const pieces = []
  let used = 0
  for (const raw of picked.length ? picked : [excerptForTool(clean, limits.maxExcerptChars)]) {
...
export function createEvidencePool(config) {
  ...
  return {
     ...
     */
    ingestSearch({ questionId, round, results, query = null, engines = [], complexity = null }) {
      const created = []
      const changed = []
      let textless = 0
      for (const hit of results ?? []) {
        ...
        if (!source) continue
        if (typeof hit.snippet === 'string' && collapseSpace(hit.snippet).length > 0 && !isBoilerplate(hit.snippet)) {
        ...
        if (!assoc) {
          ...
          assoc = {
            ...
            round,
            origin: 'search',
            provenance: [{ round, query, engines: [...engines], complexity }],
            changeCount: 0,
            judgment: null,
        ...
        if (!assoc.provenance.some((p) => p.round === round && p.query === query)) assoc.provenance.push({ round, query, engines: [...engines], complexity })
