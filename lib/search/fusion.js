...
// Free HTML scrapes + the keyless Exa MCP run on every tier.
...
//                          yahoo / bing / exa-free (each live-probed).
...
export const TIER_ENGINES = {
  ...
  complex: ['bing', 'ddg', 'yahoo', 'exa-free', 'tavily', 'brave', 'exa'],
}
...
const ENGINE_WEIGHT = { bing: 1.0, ddg: 1.05, yahoo: 1.0, 'exa-free': 1.1, tavily: 1.2, exa: 1.2, brave: 1.1 }
 ...
 */
export async function fusedSearch({ query, queries, engines, maxResults = 6, maxResultsCap = 10, includeDomains = [], excludeDomains = [], recency, tier = 'auto', layer = null, singleKeyedPool = false, depth = null, minScore = 0, runOne, signal }) {
  const started = Date.now()
  const requestedMax = Number(maxResults)
  const cap = Math.min(Math.max(1, Number(maxResultsCap) || 10), 20)
  maxResults = Math.min(Math.max(1, Number.isFinite(requestedMax) ? requestedMax : 6), cap)
  const resolvedTier = tier === 'auto' ? estimateComplexity(query) : tier
  // tavily search depth: advanced (2x credits, full text) only pays off when
  // the content will actually be consumed — complex tier or explicit override
  ...
  for (const r of discounted) {
    if (r.score < 0 || r.score < scoreFloor) continue
