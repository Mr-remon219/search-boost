// Page request bounds and HTTP URL validation. Destination DNS/address policy
// belongs to the user's local network/firewall and configured proxy. Legacy IP
// helpers below remain exported for compatibility, but fetch_page does not use
// them to pre-resolve, block private targets or pin normal direct connections.

import { BlockList, isIP } from 'node:net'
import { pageFetch } from './ipv4-fetch.js'
import { NET_ERROR_KINDS, NetworkPolicyError, isNetworkPolicyError, lookupBounded, classifyFetchError, proxyRouteFor } from './net-policy.mjs'

// Keep final-hop metadata out of the public result shape and native Response.
const responseLocations = new WeakMap()
export function pageResponseUrl(response) { return responseLocations.get(response) ?? response.url ?? '' }
...
export async function assertPublicHttpUrl(url, options = {}) {
  ...
  if (blockedHost(parsed.hostname)) throw new SsrfError(`blocked host ${parsed.hostname}`, NET_ERROR_KINDS.blockedHost)
  if (isIP(host) && isBlockedAddress(host)) throw new SsrfError(`blocked address ${host}`, NET_ERROR_KINDS.blockedAddress)
  if (isIP(host)) {
 ...
 * route re-selected: proxy or direct, without a local destination blocklist.
 * @param {string} url
...
export async function guardedFetch(url, options = {}) {
  const { headers, timeoutMs = 20_000, signal = null, maxHops = 5, env, transport } = options
  const combined = deadlineSignal(signal, timeoutMs)
  let current = String(url).trim()
  let directFallback = false
  for (let hop = 0; hop < maxHops; hop++) {
    if (combined.aborted) {
      throw new SsrfError('fetch_page: cancelled before the next hop', signal?.aborted && signal.reason?.name !== 'TimeoutError' ? NET_ERROR_KINDS.cancelled : NET_ERROR_KINDS.deadline)
    }
    const parsed = assertStaticHttpUrl(current)
    const route = directFallback ? { route: 'direct', proxyUrl: null } : proxyRouteFor(parsed, env)
    if (combined.aborted) {
      throw new SsrfError('fetch_page: cancelled before connecting', signal?.aborted && signal.reason?.name !== 'TimeoutError' ? NET_ERROR_KINDS.cancelled : NET_ERROR_KINDS.deadline)
    }
    let res
    try {
      res = await pageFetch(current, {
        route, headers, signal: combined, transport,
        onDirectFallback: () => { directFallback = true },
      })
    } catch (err) {
      if (combined.aborted) {
        throw new SsrfError('fetch_page: request interrupted', signal?.aborted && signal.reason?.name !== 'TimeoutError' ? NET_ERROR_KINDS.cancelled : NET_ERROR_KINDS.deadline)
      }
      ...
      const kind = combined.aborted
        ? (signal?.aborted && signal.reason?.name !== 'TimeoutError' ? NET_ERROR_KINDS.cancelled : NET_ERROR_KINDS.deadline)
        : classified.kind
    ...
    responseLocations.set(res, current)
    return res
  }
