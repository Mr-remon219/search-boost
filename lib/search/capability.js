...
export function formatRuntimeCapabilities(info = collectRuntimeCapabilities()) {
  ...
  return [
    ...
    ...(adaptive.configured
      ? [`adaptive_search is available (Jev source: ${adaptive.source}${adaptive.gateway === 'custom' ? ', custom gateway' : ''}). For keyword-target research, it selects engines and judges material, returning approved URLs/descriptions with cursor pagination and an overall coverage indicator; task text and the necessary evidence fragments go to ${adaptive.destination}. Use fused_search / fetch_page / x_search when you want to control the query and engines yourself.`]
      : []),
    '</search_capabilities>',
  ].join('\n')
}
