...
export function looksLikeHtml(raw) {
  ...
  // Markdown shows HTML examples in indented code blocks. When the document also
  // has markdown structure, only unindented markers may call it HTML — otherwise a
  // doc titled "# HTML example" with an indented <html> block was treated as a page
  // and the example was stripped as chrome. Indented lines are NOT protected inside
  // the HTML path itself: there the indentation is layout and its chrome must go.
  const looksMarkdown = /^#{1,6}\s/m.test(text) || /^\s*(?:[-*]|\d+\.)\s/m.test(text) || /^[ \t]{0,3}(?:`{3,}|~{3,})/m.test(text)
  const markerSource = looksMarkdown ? text.split('\n').filter((line) => !/^(?: {4,}|\t)/.test(line)).join('\n') : text
  const head = markerSource.slice(0, 4000).toLowerCase()
 ...
 * @param {{ format?: 'html'|'markdown' }} [opts] caller-known format; omit to auto-detect
 */
export function preprocessPage(raw, baseUrl, { format } = {}) {
  const input = String(raw ?? '').replace(/\r\n/g, '\n').replace(/\r/g, '\n')
  if (!input.trim()) return ''
  // Callers that already know the source should say so: Jina is asked for markdown,
  // the local fallback fetches the page itself (HTML). Sniffing is only for input
  // whose origin is unknown — and it used to misread indented markdown examples.
  if (format === 'html' || (format !== 'markdown' && looksLikeHtml(input))) {
