...
function protectCode(s, { indented = false, pre = false } = {}) {
  ...
  if (pre) {
    // restoreCode runs once, so resolve slots captured inside <pre> before stashing.
    text = text.replace(/<pre\b[^>]*>([\s\S]*?)<\/pre>/gi, (_, inner) => stash(restoreCode(preToFence(inner), slots)))
  }
  return { text, slots }
}
