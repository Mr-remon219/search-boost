...
//
// Pipeline:
//   protect fenced / pre / inline code
//     → drop chrome (outside protected regions)
...
/** Turn <pre> into fenced markdown so later chrome-stripping cannot touch it. */
function extractPreToFences(html) {
...
function htmlToReadable(html, baseUrl) {
  let s = dropComments(html)
  s = extractPreToFences(s)
  const guarded = protectCode(s)
  s = dropNonContent(guarded.text)
