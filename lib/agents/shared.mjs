...
export async function removeGeminiSnippetBlock(path) {
  await writeTextFile(path, removeGeminiBlock(await readTextFile(path)))
}

/**
 * @param {string} agentId
 * @param {string} destPath
 * @param {{ forWorkspace?: boolean }} [opts]
 */
export async function injectSkill(agentId, destPath, opts = {}) {
  const skill = await loadAgentSkill(agentId)
  if (!skill) throw new Error(`No skill template for agent: ${agentId}`)

  let header
  if (agentId === 'antigravity') {
    const route = getRoute('antigravity')
    const description = route.skillDescription ?? ''
...
export async function installAntigravityHook(workspaceRoot, dryRun) {
  const paths = {
    hooks: join(workspaceRoot, '.agents', 'hooks.json'),
    hookScript: join(workspaceRoot, '.agents', 'hooks', 'search-boost-pre-invocation.mjs'),
  }
