...
import { installStartupHook, uninstallStartupHook, STARTUP_HOOK_KEY } from '../startup-hooks.mjs'
import {
  extensionSkillRoutes,
  geminiSnippetPath,
  getRoute,
  openaiYamlPath,
  promptPath,
...
const ANTIGRAVITY_RULE_DESCRIPTION =
  'Search-boost MCP capabilities and skill entry point.'

const HOOK_ENTRY_KEY = STARTUP_HOOK_KEY
...
export function isOwnedSearchBoostSkill(content) {
  return content.includes('<!-- search-boost: skill -->')
    || /^# search-boost MCP @ (?:Claude Code|Codex|Cursor(?: CLI)?|Grok Build|Antigravity)\r?$/m.test(content)
}
...
export async function injectSkill(agentId, destPath) {
  const raw = await loadAgentSkill(agentId)
  const skill = raw?.replaceAll('{{EXTENSION_ROUTES}}', extensionSkillRoutes(agentId))
...
export async function installAntigravityHook(workspaceRoot, dryRun) {
  return installStartupHook(workspaceAgents(workspaceRoot), { dryRun, kind: 'antigravity' })
}
...
export async function uninstallAntigravityHook(workspaceRoot, dryRun) {
  return uninstallStartupHook(workspaceAgents(workspaceRoot), { dryRun, kind: 'antigravity' })
}
