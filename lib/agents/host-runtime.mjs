...
import { DSH_PACKAGE_NAME, PATHS, PI_OWNED_MARKER, PI_SHIM_MARKER, dshProfilesWithSearchBoost } from '../paths.mjs'
...
async function injectOwnedMarkdown(src, dest, dryRun) {
  ...
  if (existsSync(dest)) {
    ...
    if (current && !current.includes(PI_OWNED_MARKER)) return false
...
export async function installPiExtension(opts) {
  ...
  const settings = await strictJson(join(PATHS.pi.agentDir, 'settings.json'))
