...
import { loadStartupSearchPolicy } from '../startup-hooks.mjs'
import {
  clearCursorInstallState,
  forgetCursorCreatedFile,
  recordCursorCreatedFile,
} from '../cursor-install-state.mjs'

/**
 * @param {{ mergeCursorCli?: boolean, skillAgentId?: string }} opts
 */
async function loadHookInjectBody(opts) {
  const prompt = opts.mergeCursorCli
    ? await loadCursorMergedPrompt(true)
    : await loadAgentPrompt(opts.skillAgentId ?? 'cursor')
  return `${await loadStartupSearchPolicy()}\n\n${prompt}`
}
...
async function removeFileAndEmptyParent(filePath) {
  if (!(await removeFileIfExists(filePath))) return
  await rmdirIfEmpty(dirname(filePath))
}
...
export async function installCursorSurface(opts) {
  ...
  if (!opts.dryRun && mcpResult.action === 'created') {
    await recordCursorCreatedFile(mcpResult.path, !!opts.dryRun)
  }

  files.push(...await installSkillBundle(skillId, CURSOR_SURFACE.skill, opts))
  ...
  const hookResult = await upsertSessionStartHook(
    CURSOR_SURFACE.hooks,
    hookCmd,
    !!opts.dryRun,
    CURSOR_SURFACE.hookScript,
  )
...
export async function uninstallCursorSurface(opts) {
  await uninstallSkillBundle('cursor', CURSOR_SURFACE.skill, opts)
  await removeMcpServer(CURSOR_SURFACE.mcp, MCP_SERVER_ID, !!opts.dryRun)
  if (!opts.dryRun) {
    await forgetCursorCreatedFile(CURSOR_SURFACE.mcp, !!opts.dryRun)
    await removeFileAndEmptyParent(CURSOR_SURFACE.hookScript)
    await removeFileAndEmptyParent(CURSOR_SURFACE.hookInject)
    await rmdirIfEmpty(dirname(CURSOR_SURFACE.hookScript))
  ...
  if (!opts.dryRun) await clearCursorInstallState(!!opts.dryRun)
}
