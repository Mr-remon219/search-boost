 ...
 */
import { recordUpgradeProject } from '../upgrade/state.mjs'
...
async function removeClaudePermissions(dryRun) {
  ...
  // Unlink when our entries were the file's only content: settings.json that
  // exists solely for search-boost must not survive uninstall as `{}`.
  if (!dryRun) await writeJsonOrUnlink(file, cleaned)
}
...
async function removeAntigravityPermissions(dryRun) {
  ...
  if (!dryRun) await writeJsonOrUnlink(file, next)
}
...
export const AGENTS = {
  ...
  grok: {
    ...
    async install(opts) {
      const plugin = await installGrokPlugin({ dryRun: !!opts.dryRun, skip: !!opts.skipGrokPlugin })
      ...
      if (!opts.dryRun) {
        ...
        if (opts.scope === 'project') await recordUpgradeProject(process.cwd())
      }
      if (!plugin.ok) throw new Error('Grok MCP/config/rule/skill installed, but plugin installation failed; retry the plugin command or use --skip-grok-plugin')
      return files
    },
    async uninstall(opts) {
      ...
      const plugin = await uninstallGrokPlugin({ dryRun: !!opts.dryRun, skip: !!opts.skipGrokPlugin })
      if (!plugin.ok) throw new Error('Grok MCP removed, but plugin removal failed; retry the plugin uninstall')
    },
    printConfig: (opts) =>
