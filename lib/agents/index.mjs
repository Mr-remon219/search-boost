...
async function removeClaudePermissions(dryRun) {
  ...
  const cleaned = prunePermissions(applyClaudeNativeSettings(next, false))
...
export const AGENTS = {
  ...
  grok: {
    ...
    async install(opts) {
      ...
      toml = opts.autoAllow
        ? injectMarkedTomlSection(toml, 'permission', grokPermissionTomlBlock())
        : removeMarkedTomlSection(toml, 'permission')
    ...
    printConfig: (opts) => formatPrintConfig('grok', PATHS.grok.config, opts),
  },
