...
import { migrateCodexWebSearch } from '../codex-toml.mjs'
import { preferredPermissions } from '../antigravity-settings.mjs'
import {
...
async function writeClaudePermissions(dryRun) {
  ...
  if (claudeBypassPermissionsMode(settings)) return file
  settings.permissions ??= {}
  settings.permissions.allow ??= []
  const before = [...settings.permissions.allow]
  settings.permissions.allow = stripLegacySearchBoostAllow(settings.permissions.allow)
  settings.permissions.allow = consolidateSearchBoostAllow([
    ...settings.permissions.allow,
    ...claudePermissions(),
  ])
...
function antigravitySettingsPaths() {
  /** @type {string[]} */
  const paths = []
  if (existsSync(PATHS.antigravity.settingsCli)) paths.push(PATHS.antigravity.settingsCli)
  if (existsSync(PATHS.antigravity.settingsConfig)) paths.push(PATHS.antigravity.settingsConfig)
  if (paths.length === 0) paths.push(preferredAntigravitySettingsPath())
  return paths
}

async function writeAntigravityPermissions(dryRun) {
  let touched = null
  for (const file of antigravitySettingsPaths()) {
    const settings = await readJsonFile(file, {})
    settings.permissions ??= {}
    settings.permissions.allow ??= []
    const before = [...settings.permissions.allow]
    ...
    if (jsonDeepEqual(before, settings.permissions.allow)) continue
    if (!dryRun) await writeJsonFile(file, settings)
    touched ??= file
  }
  return touched ?? preferredAntigravitySettingsPath()
}
...
async function removeAntigravityPermissions(dryRun) {
  for (const file of antigravitySettingsPaths()) {
    ...
    const settings = await readJsonFile(file, {})
    if (!Array.isArray(settings.permissions?.allow)) continue
    ...
    if (jsonDeepEqual(settings, next)) continue
    if (!dryRun) await writeJsonFile(file, next)
  }
}
