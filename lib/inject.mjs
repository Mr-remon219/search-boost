...
export function injectGeminiBlock(existing, snippet) {
  const block = `${GEMINI_MARKER_START}\n${snippet.trim()}\n${GEMINI_MARKER_END}`
  if (existing.includes(GEMINI_MARKER_START)) {
    const before = existing.slice(0, existing.indexOf(GEMINI_MARKER_START))
    const after = existing.slice(existing.indexOf(GEMINI_MARKER_END) + GEMINI_MARKER_END.length)
    return `${before}${block}${after}`.replace(/\n{3,}/g, '\n\n').trim() + '\n'
  }
  const header = existing.trim() ? `${existing.trim()}\n\n` : ''
  return `${header}${block}\n`
...
export function removeGeminiBlock(existing) {
  if (!existing.includes(GEMINI_MARKER_START)) return existing
  const before = existing.slice(0, existing.indexOf(GEMINI_MARKER_START))
  const after = existing.slice(existing.indexOf(GEMINI_MARKER_END) + GEMINI_MARKER_END.length)
  const out = `${before}${after}`.replace(/\n{3,}/g, '\n\n').trim()
  return out ? `${out}\n` : ''
