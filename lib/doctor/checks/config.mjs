...
import { ENV_MAP, KEY_NAMES, keysFilePath, readKeysFile, readKeysRouting, RECOMMEND_ALL_KEYED_ENGINES } from '../../keys.mjs'
...
export function checkConfigPathsWritable(_ctx) {
  ...
  const usingOverrides = !!(process.env.SEARCH_BOOST_KEYS_FILE || process.env.SEARCH_BOOST_LAYER_FILE)
  try {
    ...
    return {
      ...
      category: 'config',
      status: 'pass',
      message: usingOverrides
        ? 'Config path overrides writable'
    ...
    return {
      ...
      status: 'fail',
      message: usingOverrides
        ? 'SEARCH_BOOST_*_FILE parent directory not writable'
        : 'Home directory not writable for config files',
      fix_hint: usingOverrides
        ? 'Fix permissions on override paths or unset SEARCH_BOOST_*_FILE'
        : 'Fix permissions or set SEARCH_BOOST_KEYS_FILE',
