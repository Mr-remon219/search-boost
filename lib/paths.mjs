import { existsSync, readdirSync, readFileSync } from 'node:fs'
import { homedir } from 'node:os'
import { cwd } from 'node:process'
import { join, dirname } from 'node:path'
import { getPackageName } from './pkg.mjs'
import { PI_PACKAGE_NAMES, packageIdentity } from './package-identity.mjs'
import { RETIRED_SKILL_NAMES, extensionSkills } from '../agents/router.mjs'

export const HOME = homedir()
export const MCP_SERVER_ID = 'search-boost'

const GEMINI_CONFIG = join(HOME, '.gemini', 'config')
const GEMINI_LEGACY = join(HOME, '.gemini', 'antigravity')

/** Shared Cursor IDE + CLI surface paths. */
export const CURSOR_SURFACE = {
  mcp: join(HOME, '.cursor', 'mcp.json'),
  /** Legacy — uninstall sweeps only; install no longer writes here. */
  agents: join(HOME, '.cursor', 'AGENTS.md'),
  skill: join(HOME, '.cursor', 'skills', 'search-boost', 'SKILL.md'),
  hooks: join(HOME, '.cursor', 'hooks.json'),
  hookScript: join(HOME, '.cursor', 'hooks', 'search-boost-session.mjs'),
  hookInject: join(HOME, '.cursor', 'hooks', 'search-boost-inject.md'),
  cliConfig: join(HOME, '.cursor', 'cli-config.json'),
}

export const PATHS = {
  cursor: { ...CURSOR_SURFACE },
  'cursor-cli': { ...CURSOR_SURFACE },
  codex: {
    config: join(HOME, '.codex', 'config.toml'),
    hooks: join(HOME, '.codex', 'hooks.json'),
    hookScript: join(HOME, '.codex', 'hooks', 'search-boost-session.mjs'),
    hookInject: join(HOME, '.codex', 'hooks', 'search-boost-inject.md'),
    agents: join(HOME, '.codex', 'AGENTS.md'),
    skill: join(HOME, '.agents', 'skills', 'search-boost', 'SKILL.md'),
    openaiYaml: join(HOME, '.agents', 'skills', 'search-boost', 'agents', 'openai.yaml'),
  },
  claude: {
    config: join(HOME, '.claude.json'),
    settings: join(HOME, '.claude', 'settings.json'),
    hooks: join(HOME, '.claude', 'settings.json'),
    hookScript: join(HOME, '.claude', 'hooks', 'search-boost-session.mjs'),
    hookInject: join(HOME, '.claude', 'hooks', 'search-boost-inject.md'),
    agents: join(HOME, '.claude', 'CLAUDE.md'),
    skill: join(HOME, '.claude', 'skills', 'search-boost', 'SKILL.md'),
  },
  grok: {
    config: join(HOME, '.grok', 'config.toml'),
    rule: join(HOME, '.grok', 'rules', 'search-boost.md'),
    skill: join(HOME, '.grok', 'skills', 'search-boost', 'SKILL.md'),
  },
  'grok-project': {
    config: join(cwd(), '.grok', 'config.toml'),
    skill: join(cwd(), '.grok', 'skills', 'search-boost', 'SKILL.md'),
  },
  antigravity: {
    /** Resolved at runtime via preferredAntigravityMcpPath() */
    mcp: join(GEMINI_CONFIG, 'mcp_config.json'),
    legacyMcp: join(GEMINI_LEGACY, 'mcp_config.json'),
    migratedMarker: join(GEMINI_CONFIG, '.migrated'),
    agents: join(HOME, '.gemini', 'AGENTS.md'),
    gemini: join(HOME, '.gemini', 'GEMINI.md'),
    /** Cross-platform skill path (AGY IDE + AGY CLI + Gemini CLI) */
    skill: join(GEMINI_CONFIG, 'skills', 'search-boost', 'SKILL.md'),
    settingsCli: join(HOME, '.gemini', 'antigravity-cli', 'settings.json'),
    settingsConfig: join(GEMINI_CONFIG, 'settings.json'),
    hooks: join(GEMINI_CONFIG, 'hooks.json'),
    hookScript: join(GEMINI_CONFIG, 'hooks', 'search-boost-pre-invocation.mjs'),
    hookInject: join(GEMINI_CONFIG, 'hooks', 'search-boost-inject.md'),
  },
  pi: {
    /** pi agent dir (PI_CODING_AGENT_DIR override honoured at runtime via piAgentDir()) */
    agentDir: piAgentDirPath(),
    /** Extension shim discovered by pi (extensions/*.js) — re-exports adapters/pi/index.js */
    extension: join(piAgentDirPath(), 'extensions', 'search-boost.js'),
    /** Legacy pi-search-boost manual copy (never deleted by us; flagged as a conflict) */
    legacyExtensionDir: join(piAgentDirPath(), 'extensions', 'search-boost'),
    /** Injected subagent defs (searcher.md / summarizer.md) */
    agentsDir: join(piAgentDirPath(), 'agents'),
    /** Injected slash prompts (fast-parallel.md / complex-parallel.md) */
    promptsDir: join(piAgentDirPath(), 'prompts'),
  },
  dsh: {
    home: dshHomePath(),
    profiles: join(dshHomePath(), 'profiles'),
  },
}

function piAgentDirPath() {
  const envDir = process.env.PI_CODING_AGENT_DIR
  if (envDir) return envDir.replace(/^~(?=$|[\\/])/, HOME)
  return join(HOME, '.pi', 'agent')
}

function dshHomePath() {
  const envDir = process.env.DSH_HOME
  if (envDir) return envDir.replace(/^~(?=$|[\\/])/, HOME)
  return join(HOME, '.dsh')
}

/** `$DSH_HOME/profiles/<name>` — where `dsh plugin --profile <name>` installs bundles. */
export function dshProfileDir(profile = 'web') {
  return join(PATHS.dsh.profiles, profile)
}

/** DSH profiles whose package.json depends on this package (bundle installed). */
export function dshProfilesWithSearchBoost() {
  const out = []
  try {
    for (const entry of readdirSync(PATHS.dsh.profiles, { withFileTypes: true })) {
      if (!entry.isDirectory()) continue
      const pkgPath = join(PATHS.dsh.profiles, entry.name, 'package.json')
      try {
        const pkg = JSON.parse(readFileSync(pkgPath, 'utf8'))
        const deps = { ...(pkg.dependencies ?? {}), ...(pkg.devDependencies ?? {}) }
        const bundles = pkg.dsh?.profile?.bundles ?? []
        if (deps[DSH_PACKAGE_NAME] || bundles.includes(DSH_PACKAGE_NAME)) out.push(entry.name)
      } catch { /* not a profile */ }
    }
  } catch { /* no profiles dir */ }
  return out
}

/** npm package name DSH records in the profile manifest. */
export const DSH_PACKAGE_NAME = getPackageName()

/** Marker line in the pi extension shim so uninstall only removes our own file. */
export const PI_SHIM_MARKER = 'search-boost pi extension shim'

/** Frontmatter fingerprint on injected pi agent/prompt markdown. */
export const PI_OWNED_MARKER = 'search-boost: owned'

/** @param {'user'|'project'} [scope] */
export function grokInstallPaths(scope = 'user') {
  if (scope === 'project') {
    const root = cwd()
    return {
      config: join(root, '.grok', 'config.toml'),
      rule: join(root, '.grok', 'rules', 'search-boost.md'),
      skill: join(root, '.grok', 'skills', 'search-boost', 'SKILL.md'),
    }
  }
  return PATHS.grok
}

/** User + project Grok config paths (for configured checks). */
export function grokConfigCandidates() {
  return [PATHS.grok.config, grokInstallPaths('project').config]
}

/** @param {'user'|'project'} scope */
export function grokScopeConfigured(scope) {
  try {
    const path = grokInstallPaths(scope).config
    if (!existsSync(path)) return false
    const toml = readFileSync(path, 'utf8')
    return toml.includes(`[mcp_servers.${MCP_SERVER_ID}]`)
  } catch {
    return false
  }
}

/** @param {'user'|'project'} scope — config, rule, skill, or permission artifacts */
export function grokScopeHasArtifacts(scope) {
  const paths = grokInstallPaths(scope)
  if ([...RETIRED_SKILL_NAMES, ...extensionSkills('grok').map((entry) => entry.name)].some((name) => existsSync(join(dirname(dirname(paths.skill)), name, 'SKILL.md')))) return true
  if (existsSync(paths.rule)) return true
  if (existsSync(paths.skill)) return true
  if (!existsSync(paths.config)) return false
  try {
    const toml = readFileSync(paths.config, 'utf8')
    if (toml.includes(`[mcp_servers.${MCP_SERVER_ID}]`)) return true
    if (toml.includes('SEARCH_BOOST_permission_START')) return true
    if (/MCPTool\(search-boost__/.test(toml)) return true
    return false
  } catch {
    return false
  }
}

/** @param {'user'|'project'|'all'} [scope] */
export function grokUninstallScopes(scope = 'user') {
  if (scope === 'all') return ['user', 'project']
  if (scope === 'project') return ['project']
  return ['user']
}

/** Antigravity unified vs legacy MCP config path (codegraph-compatible). */
export function preferredAntigravityMcpPath() {
  if (existsSync(PATHS.antigravity.migratedMarker)) return PATHS.antigravity.mcp
  if (existsSync(PATHS.antigravity.mcp)) return PATHS.antigravity.mcp
  return PATHS.antigravity.legacyMcp
}

/** All Antigravity MCP paths to sweep on uninstall. */
export function antigravityMcpPaths() {
  const preferred = preferredAntigravityMcpPath()
  const other = preferred === PATHS.antigravity.mcp
    ? PATHS.antigravity.legacyMcp
    : PATHS.antigravity.mcp
  return preferred === other ? [preferred] : [preferred, other]
}

/** Antigravity permissions settings — prefer CLI path if it exists. */
export function preferredAntigravitySettingsPath() {
  if (existsSync(PATHS.antigravity.settingsCli)) return PATHS.antigravity.settingsCli
  if (existsSync(PATHS.antigravity.settingsConfig)) return PATHS.antigravity.settingsConfig
  return PATHS.antigravity.settingsCli
}

/** Workspace `.agents/` inject paths under a project root. */
export function workspaceAgents(cwd) {
  const root = cwd
  return {
    mcp: join(root, '.agents', 'mcp_config.json'),
    skill: join(root, '.agents', 'skills', 'search-boost', 'SKILL.md'),
    rule: join(root, '.agents', 'rules', 'search-boost.md'),
    hooks: join(root, '.agents', 'hooks.json'),
    hookScript: join(root, '.agents', 'hooks', 'search-boost-pre-invocation.mjs'),
    hookInject: join(root, '.agents', 'hooks', 'search-boost-inject.md'),
  }
}

/** @param {string} id */
export function agentDetected(id) {
  switch (id) {
    case 'cursor':
    case 'cursor-cli':
      return existsSync(join(HOME, '.cursor'))
    case 'codex':
      return existsSync(join(HOME, '.codex'))
    case 'claude':
      return existsSync(join(HOME, '.claude')) || existsSync(PATHS.claude.config)
    case 'grok':
      return existsSync(join(HOME, '.grok'))
    case 'antigravity':
      return (
        existsSync(join(HOME, '.gemini'))
        || existsSync(join(HOME, '.antigravity'))
        || existsSync(GEMINI_CONFIG)
        || existsSync(GEMINI_LEGACY)
      )
    case 'pi':
      return existsSync(join(HOME, '.pi')) || existsSync(PATHS.pi.agentDir)
    case 'dsh':
      return existsSync(PATHS.dsh.home)
    default:
      return false
  }
}

/** @param {string} id */
export function agentConfigured(id) {
  try {
    switch (id) {
      case 'cursor':
      case 'cursor-cli': {
        const cfg = JSON.parse(readFileSync(PATHS.cursor.mcp, 'utf8'))
        return !!cfg.mcpServers?.[MCP_SERVER_ID]
      }
      case 'claude': {
        const cfg = JSON.parse(readFileSync(PATHS.claude.config, 'utf8'))
        return !!cfg.mcpServers?.[MCP_SERVER_ID]
      }
      case 'antigravity': {
        for (const p of antigravityMcpPaths()) {
          try {
            if (JSON.parse(readFileSync(p, 'utf8')).mcpServers?.[MCP_SERVER_ID]) return true
          } catch { /* next */ }
        }
        return false
      }
      case 'codex': {
        const path = PATHS.codex.config
        const toml = readFileSync(path, 'utf8')
        return toml.includes(`[mcp_servers.${MCP_SERVER_ID}]`)
      }
      case 'grok': {
        for (const path of grokConfigCandidates()) {
          try {
            const toml = readFileSync(path, 'utf8')
            if (toml.includes(`[mcp_servers.${MCP_SERVER_ID}]`)) return true
          } catch { /* next */ }
        }
        return false
      }
      case 'pi': {
        if (existsSync(PATHS.pi.extension) && readFileSync(PATHS.pi.extension, 'utf8').includes(PI_SHIM_MARKER)) return true
        // A legacy package name (`pi-search-boost`, `search-boost-mcp`) exposes the
        // same tools, so it counts as configured: otherwise the doctor tells the user
        // to install again and two extensions end up registering duplicate tools.
        return piSettingsIdentities().some((identity) => PI_PACKAGE_NAMES.includes(identity))
      }
      case 'dsh':
        return dshProfilesWithSearchBoost().length > 0
      default:
        return false
    }
  } catch {
    return false
  }
}

/** Identities of the packages/extensions registered in pi's settings. */
function piSettingsIdentities() {
  const settings = JSON.parse(readFileSync(join(PATHS.pi.agentDir, 'settings.json'), 'utf8'))
  return ['packages', 'extensions'].flatMap((field) => (Array.isArray(settings[field]) ? settings[field] : [])
    .map((entry) => packageIdentity(typeof entry === 'string' ? entry : entry?.source, PATHS.pi.agentDir)))
}

/**
 * True when pi registers a legacy search-boost package rather than the current
 * one: those tools are already live through the old copy, so the user needs
 * `pi remove pi-search-boost` or an upgrade — not another install.
 */
export function piRegistersLegacyPackage() {
  try {
    return piSettingsIdentities().some((identity) => PI_PACKAGE_NAMES.includes(identity) && identity !== DSH_PACKAGE_NAME)
  } catch {
    return false
  }
}
