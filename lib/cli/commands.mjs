...
import { runXAuthWizard } from '../installer/xauth-wizard.mjs'
import { runJevWizard } from '../installer/jev-wizard.mjs'
...
export async function runConfigKeys(args) {
  ...
  if (flags.yes || flags.show || Object.keys(flags.set).length || flags.unset.length
    || flags.engines !== null || flags.enable.length || flags.disable.length) {
...
/** @param {string[]} args */
export async function runConfigX(args) {
  const flags = parseFlags(args)
  if (flags.help) {
    help()
    return
  }
  if (flags.yes || flags.show || flags.importGrok || flags.setXaiKey != null || flags.logout) {
    await runXAuthWizard(null, {
      show: flags.show,
      importGrok: flags.importGrok,
      setXaiKey: flags.setXaiKey,
      logout: flags.logout,
      yes: flags.yes,
      dryRun: flags.dryRun,
    })
  ...
  const clack = await loadClack()
  await runXAuthWizard(clack, flags)
}

/** @param {string[]} args */
export async function runConfigJev(args) {
  const flags = parseFlags(args)
  if (flags.help) {
    help()
    return
  }
  if (flags.yes || flags.show || flags.clear || flags.jevBaseUrl != null || flags.jevApiKey != null) {
    await runJevWizard(null, {
      show: flags.show,
      clear: flags.clear,
      setBaseUrl: flags.jevBaseUrl,
      setApiKey: flags.jevApiKey,
      yes: flags.yes,
      dryRun: flags.dryRun,
    })
    return
  ...
  const clack = await loadClack()
  await runJevWizard(clack, flags)
}
...
export async function runConfigLayer(args) {
  ...
  if (flags.layer === 'free' || flags.layer === 'api') {
    if (flags.dryRun) {
      console.log(`dry-run: would set the layer to ${flags.layer} (nothing written)`)
      return
    }
  ...
  if (flags.dryRun) {
    console.log(`dry-run: would set the layer to ${layer} (nothing written)`)
    return
  }
