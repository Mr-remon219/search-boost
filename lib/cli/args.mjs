 ...
 * @property {boolean} importGrok
 * @property {string|null} setXaiKey
 * @property {boolean} logout
 * @property {string|null} jevBaseUrl
 * @property {string|null} jevApiKey
...
export function parseFlags(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (a === '-y' || a === '--yes') flags.yes = true
      ...
      flags.engines = takeValue(args, i, a)
      i += 1
    } else if (a === '--enable') {
      flags.enable.push(takeValue(args, i, a))
      i += 1
    } else if (a === '--disable') {
      flags.disable.push(takeValue(args, i, a))
      i += 1
    } else if (a === '--skip-grok-plugin') {
      flags.skipGrokPlugin = true
    } else if (a === '--profile') {
      flags.profile = takeValue(args, i, a)
      i += 1
    } else if (a === '--import-grok') {
      flags.importGrok = true
    } else if (a === '--set-xai-key') {
      flags.setXaiKey = takeValue(args, i, a)
      i += 1
    } else if (a === '--logout') {
      flags.logout = true
    } else if (a === '--jev-base-url') {
      flags.jevBaseUrl = takeValue(args, i, a)
      i += 1
    } else if (a === '--jev-api-key') {
      flags.jevApiKey = takeValue(args, i, a)
      i += 1
    } else if (a === '--clear') {
      flags.clear = true
    } else {
...
export function installOpts(flags, uninstall = false) {
  return {
    ...
    skipGrokPlugin: flags.skipGrokPlugin,
...
export function help() {
  ...
  console.log(`search-boost — multi-engine search for coding agents (MCP server, pi extension, DSH bundle)

Usage:
  search-boost                         Interactive TUI
  search-boost setup                   Full onboarding (keys + layer + agents)
  search-boost migrate [-y] [--dry-run] [--workspace PATH]  Legacy npm rename only
  search-boost upgrade [-y] [--dry-run] [--sync-only] [--workspace PATH]
  search-boost install [options]
  search-boost uninstall [options]
  search-boost config keys|layer|x|jev|search|diag
...
Install / setup options:
  -t, --target <ids>     cursor,codex,claude,grok,antigravity,cursor-cli,pi,dsh | auto | all
  ...
  --skip-grok-plugin     Grok only: skip bundled grok plugin install (config/rule/skill only)
...
Config keys:
  --show                 Print masked key status and api-engine routing
  --set tavily=KEY       Write a key to ~/.search-boost/config/keys.json
  --set exa=KEY          (also: brave=KEY)
  --set anysearch=KEY    Store the anysearch key (key slot only — no adapter/api-pool wiring yet)
  --unset brave          Remove a key from the file
  --engines exa          Limit api layer to exa (comma-separated: exa,brave)
  --engines all          Clear enabledEngines — use all configured keys
  --enable exa           Enable one keyed engine in routing
  --disable brave        Disable one keyed engine in routing

Config layer:
  --show                 Print current layer
  --layer free|api       Persist default layer

Config x (X/Twitter credentials for official x_search):
  --show                 Print xauth status
  --import-grok          Import grok CLI login into local copy
  --set-xai-key KEY      Store XAI API key (public api.x.ai)
  --logout               Remove local xauth copy

Config jev (experimental — TypeSafe Jev System One decision model):
  --show                 Print Jev status
  --jev-base-url URL     Endpoint base (default https://api.typesafe.ai/v1)
  --jev-api-key KEY      Store the Jev API key (console.typesafe.ai)
  --clear                Remove stored Jev credentials

  When configured, the optional adaptive_search tool uses Jev to select engines,
  judge evidence and return approved URLs/descriptions with pagination. It sends task
  text and the necessary evidence fragments to that endpoint (default TypeSafe);
  fused_search / fetch_page / x_search stay fully usable without Jev.
...
Agents:
${agents}
`.replace(/^  search-boost(?=\s|$)/gm, `  ${getPackageName()}`))
}
