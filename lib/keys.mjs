...
function envKeyName(name) {
  return process.env[ENV_MAP[name]]?.trim() ? ENV_MAP[name] : LEGACY_ENV_MAP[name]
}
function envKey(name) { return process.env[envKeyName(name)]?.trim() }
...
function keysReadCandidates(options = {}) {
  ...
  if (legacy && !existsSync(flat) && !keysFileHasValues(nested) && !(envPath && existsSync(envPath))) {
...
/** @param {string} file */
function keysFileHasValues(file) {
  if (!existsSync(file)) return false
  try {
    return keysDocHasValues(JSON.parse(readFileSync(file, 'utf8')))
  } catch {
    return false
  }
}

/** Flat/env/cwd paths are terminal when present (even empty); nested only when it has keys. */
/** @param {{ homeDir?: string }} [options] */
function keysTerminalWhenExists(options = {}) {
  ...
  const nested = configNestedPath('keys', options)
  if (keysFileHasValues(nested)) set.add(nested)
  return set
}
  ...
  for (const file of candidates) {
    ...
    try {
      ...
      if (terminalWhenExists.has(file) || Object.values(keys).some(Boolean)) {
        if (file === legacy && !keysFileHasValues(nested) && !existsSync(flat)) {
