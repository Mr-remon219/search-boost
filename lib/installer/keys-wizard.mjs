...
export async function runKeysWizard(clack, opts = {}) {
  ...
  if (opts.set && Object.keys(opts.set).length > 0) {
    ...
    console.log(`Saved keys to ~/.dsh-search-boost-keys.json`)
    return
  }
  ...
  clack.log.info('Keys are stored in ~/.dsh-search-boost-keys.json (not in agent MCP configs).')
  ...
  writeKeysFile(patch)
  clack.log.success('Saved ~/.dsh-search-boost-keys.json')
}
...
export function printKeyStatus() {
  ...
  console.log(`\nFile: ~/.dsh-search-boost-keys.json`)
}
