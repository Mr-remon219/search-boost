...
export async function runKeysWizard(clack, opts = {}) {
  ...
  if (opts.engines !== undefined && opts.engines !== null) {
    if (opts.engines === 'all') {
      if (dryRun) wouldWrite('would clear enabledEngines — all configured keys would be used on the api layer')
      else setEnabledEngines(null)
      ...
      // Unknown names are rejected before anything is written; they must never be
      // read as "clear the restriction".
      const validated = assertEngineNames(names, '--engines')
      if (dryRun) wouldWrite(`would enable api-layer engines: ${validated.join(', ')}`)
      else setEnabledEngines(validated)
  ...
  if (opts.enable?.length || opts.disable?.length) {
    ...
    for (const name of opts.enable ?? []) {
      assertEngineNames([name], '--enable')
      enabled.add(name)
    }
    for (const name of opts.disable ?? []) {
      assertEngineNames([name], '--disable')
      enabled.delete(name)
    }
    // The whitelist is the explicit enable: writing it also clears a stale
    // per-engine disable flag for the names it allows.
    if (dryRun) wouldWrite(`would set api-layer engines to: ${[...enabled].join(', ') || '(none)'}`)
    else setEnabledEngines([...enabled])
    console.log(`Enabled api-layer engines: ${[...enabled].join(', ') || '(none)'}`)
    return
  }

  if (opts.set && Object.keys(opts.set).length > 0) {
    if (dryRun) wouldWrite(`would save ${Object.keys(opts.set).join(', ')} to ${tildify(keysFilePath())}`)
    else for (const [name, value] of Object.entries(opts.set)) setKey(name, value)
  ...
  if (opts.unset?.length) {
    if (dryRun) {
      wouldWrite(`would remove ${opts.unset.join(', ')} from ${tildify(keysFilePath())}`)
    } else {
  ...
  if (dryRun) {
    wouldWrite(`would save key changes to ${tildify(keysFilePath())}`)
    clack.log.info('dry-run: no credentials written')
    return
  }
