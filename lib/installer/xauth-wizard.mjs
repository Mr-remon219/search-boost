...
export async function runXAuthWizard(clack, opts = {}) {
  ...
  if (opts.importGrok) {
    ...
    if (!dryRun) console.log(`Imported grok login → ${tildify(piAuthPath())}`)
    return
  }
  ...
  if (opts.logout) {
    ...
    console.log(
      dryRun
        ? `dry-run: would remove the local xauth copy (${tildify(piAuthPath())})`
        : removed
  ...
  switch (action) {
    ...
    case 'import-grok': {
      if (dryRun) {
        ...
        clack.log.info(`dry-run: would import the grok login → ${tildify(piAuthPath())}`)
        return
      }
      try {
        importFromGrok()
    ...
    case 'set-key': {
      ...
      if (dryRun) {
        clack.log.info(`dry-run: would save an XAI API key → ${tildify(piAuthPath())}`)
        return
      }
    ...
    case 'remove': {
      if (dryRun) {
        clack.log.info(`dry-run: would remove the local xauth copy (${tildify(piAuthPath())})`)
        return
      }
