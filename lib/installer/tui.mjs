...
export async function runTui(opts = {}, { clack: suppliedClack, update = runUpgrade } = {}) {
  const clack = suppliedClack ?? await loadClack()
  ...
  while (running) {
    const action = await clack.select({
      ...
      options: [
        ...
        { value: 'upgrade', label: 'Update', hint: 'search-boost + all installed agents' },
        { value: 'uninstall', label: 'Uninstall' },
        { value: 'keys', label: 'API keys', hint: 'tavily / brave / exa / anysearch' },
        { value: 'layer', label: 'Search layer', hint: 'free or api' },
        { value: 'x', label: 'X credentials', hint: 'grok login / XAI_API_KEY' },
        { value: 'jev', label: 'Jev credentials (experimental)', hint: 'TypeSafe / Vercel URL + API key' },
    ...
    switch (action) {
      case 'setup':
        ...
        break
      case 'install':
        clack.log.info('Note: Skipped API keys, layer, and X credentials (install-only).')
        ...
        break
      case 'upgrade':
        clack.log.info('Updates search-boost and all installed agents, including pi-search-boost and dsh-search-boost adapters. User configuration, credentials and permission choices are preserved.')
        try {
          ...
          if (result.reloaded) running = false
        } catch (err) {
      ...
      case 'status': {
        noteStatus(clack)
        try {
          const inventory = await discoverIntegrations({ workspace: opts.workspace })
          clack.note(inventory.targets.map((t) => `${t.label}${t.legacy ? ' — legacy adapter, refresh via Update' : ''}`).join('\n') || 'No existing integrations found.', 'Managed / legacy integrations')
          for (const warning of inventory.warnings) clack.log.warn(warning)
        } catch (err) { clack.log.error(err.message) }
        break
      }
      case 'print':
...
async function runLayerTui(clack, opts = {}) {
  ...
  if (opts.dryRun) {
    clack.log.info(`dry-run: would set the search layer to ${layer} (nothing written)`)
    return
  }
