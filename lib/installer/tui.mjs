  ...
  while (running) {
    const action = await clack.select({
      ...
      options: [
        ...
        { value: 'upgrade', label: 'One-click upgrade / 一键升级', hint: 'check npm + migrate existing pi/dsh/MCP; keep credentials' },
    ...
    switch (action) {
      ...
      case 'install':
        ...
        break
      case 'upgrade':
        clack.log.info('Updates the npm package if older, then refreshes existing integrations, including legacy search-boost-mcp, pi-search-boost and dsh-search-boost. Credentials and permission choices are preserved.')
        try {
          const result = await runUpgrade({ dryRun: opts.dryRun, workspace: opts.workspace, log: (message) => clack.log.info(message) })
      ...
      case 'status': {
        ...
        try {
          ...
          clack.note(inventory.targets.map((t) => `${t.label}${t.legacy ? ' — legacy, migrate via One-click upgrade' : ''}`).join('\n') || 'No existing integrations found.', 'Managed / legacy integrations')
