...
async function runAutoAllowStep(clack, rawTargets, opts) {
  const needsClaude = rawTargets.includes('claude')
  const needsAntigravity = rawTargets.includes('antigravity')
  if (!needsClaude && !needsAntigravity) return false
  if (opts.autoAllow === true) return true
  if (opts.yes) return false

  const agents = [
    needsClaude ? 'Claude Code' : null,
    needsAntigravity ? 'Antigravity' : null,
  ].filter(Boolean).join(' + ')

  const ans = await clack.confirm({
    message: `Auto-allow search-boost MCP tools in ${agents}? (skips permission prompts)`,
    initialValue: true,
  })
 ...
 * @param {{ dryRun?: boolean, autoAllow?: boolean, mergeCursorCli?: boolean, uninstall?: boolean, workspace?: string|null }} opts
...
export async function runInstallPlain(opts) {
  ...
  console.log(
    `${opts.uninstall ? 'Uninstall' : 'Install'} search-boost → ${rawTargets.join(', ')}${mergeNote}${opts.dryRun ? ' (dry-run)' : ''}${opts.workspace ? ` workspace=${opts.workspace}` : ''}\n`,
  )
  await executeAgentOps(targets, {
    dryRun: !!opts.dryRun,
