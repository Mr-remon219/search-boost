...
async function runAutoAllowStep(clack, rawTargets, opts) {
  if (!rawTargets.includes('claude')) return false
  if (opts.autoAllow === true) return true
  if (opts.yes) return true

  const ans = await clack.confirm({
    message: 'Auto-allow search-boost tools in Claude Code? (skips permission prompts)',
    initialValue: true,
  })
 ...
 * @param {{ dryRun?: boolean, autoAllow?: boolean, mergeCursorCli?: boolean, uninstall?: boolean }} opts
...
export async function runInstallPlain(opts) {
  ...
  const autoAllow = !!opts.autoAllow || (!!opts.yes && rawTargets.includes('claude'))
  console.log(
