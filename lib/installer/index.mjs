...
async function runKeysStep(clack, opts) {
  ...
  if (opts.dryRun) {
    ...
    clack.log.info('dry-run: would offer API key setup (no credentials written)')
    return
  }

  const configure = await clack.confirm({
    message: 'Configure API keys now? (one of tavily/brave/exa is enough; all three recommended; anysearch is stored for a later adapter)',
  ...
  clack.log.info('Tip: a single keyed engine works on the api layer; configure all three when you can for best fusion.')
...
async function runLayerStep(clack, opts) {
  if (opts.skipLayer || opts.yes) {
    if (opts.dryRun) {
      clack.log.info('dry-run: would initialize the default search layer (nothing written)')
      return
    }
...
async function runXAuthStep(clack, opts) {
  ...
  if (opts.dryRun) {
    clack.log.info('dry-run: would offer to import the grok login (no credentials written)')
    return
  }

  const ans = await clack.confirm({
    message: 'Import grok login for official x_search? (optional — improves X/Twitter search)',
    initialValue: true,
  })
  handleCancel(ans, clack)
  if (!ans) {
    clack.log.info('Skipped — run `search-boost config x` later to enable official x_search.')
    return
  }
  try {
    importFromGrok()
...
export async function runInstallerWithOptions(opts = {}) {
  ...
  if (rawTargets.length === 0) {
    const hint = 'Try `search-boost install --target all` or run `search-boost install` without -y/-t for interactive selection.'
    if (ownChrome) {
      clack.log.info(hint)
      clack.outro('No agents selected.')
    } else {
      clack.log.warn('No agents selected.')
      clack.log.info(hint)
    }
    return
  }
