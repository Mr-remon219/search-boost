...
export async function installStartupHook(paths, { dryRun = false, kind = 'session', preserveExisting = false } = {}) {
    ...
    config[STARTUP_HOOK_KEY] = {
      ...
      PreInvocation: [
        ...(prior?.PreInvocation ?? []).filter((h) => !ownedHandler(h, paths)
          && h?.command !== 'node ./hooks/search-boost-pre-invocation.mjs'),
        preserveExisting ? { ...handler, ...(prior?.PreInvocation ?? []).find((h) => ownedHandler(h, paths)), command: handler.command } : handler,
      ],
    }
  } else {
    config.hooks ??= {}
    const groups = config.hooks.SessionStart ?? []
    const hasOwned = groups.some((g) => g.hooks?.some((h) => ownedHandler(h, paths)))
    config.hooks.SessionStart = preserveExisting && hasOwned
      ? groups.map((g) => ({ ...g, ...(Array.isArray(g.hooks) ? { hooks: g.hooks.map((h) => ownedHandler(h, paths) ? { ...h, command: handler.command } : h) } : {}) }))
      : [...stripSessionHook(groups, paths), { hooks: [handler] }]
  }
