...
export async function readJsonFile(path, fallback) {
  try {
    ...
    throw new Error(`Cannot read JSON configuration ${path}; left unchanged (${err?.code ?? 'invalid JSON/object'})`)
  }
}

/** Host configs may be dotfile symlinks. Resolve an existing regular target and
 * atomically update it without replacing the link. Dangling/cyclic links and
 * non-files fail closed; credential stores retain their no-symlink policy. */
function hostWritePath(path) {
  let info
  ...
  if (!(info.isSymbolicLink() ? statSync(target) : info).isFile()) {
    throw new Error(`Not a regular host configuration file: ${path}; left unchanged`)
  }
  return target
}
...
export async function readTextFile(path) {
  try {
    ...
    throw new Error(`Cannot read configuration ${path}; left unchanged (${err?.code ?? 'read failure'})`)
  }
}
...
export async function writeTextFile(path, content) {
  ...
  writeFileAtomicPrivate(hostWritePath(path), content, { tightenDir: false })
}
...
export async function upsertMcpServer(path, serverId, entry, dryRun) {
  ...
  if (existing.mcpServers != null && (typeof existing.mcpServers !== 'object' || Array.isArray(existing.mcpServers))) {
    throw new Error(`Invalid mcpServers object in ${path}; left unchanged`)
  }
  existing.mcpServers ??= {}
