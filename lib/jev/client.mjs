...
import { normalizeJevBaseUrl, jevProvider, JEV_VERCEL_MODEL, JEV_DEFAULT_MODEL } from '../jev-config.mjs'
...
export class JevError extends Error {
  ...
  toJSON() {
    return {
      ...
      ...(this.retryAfter !== null ? { retryAfterMs: this.retryAfter } : {}),
...
/** Only protocol field roots and known error codes may leave an error response. */
export function safeServerDetail(body, status) {
  const codes = new Set(['unauthorized', 'forbidden', 'invalid_api_key', 'invalid_request', 'validation_error', 'rate_limited', 'rate_limit_exceeded', 'overloaded', 'server_error', 'internal_server_error', 'insufficient_credits', 'insufficient_quota'])
  try {
    const parsed = JSON.parse(body)
    const field = parsed?.detail?.field ?? parsed?.field
    if (typeof field === 'string') {
      const root = field.split(/[.\[]/, 1)[0]
      if (['state', 'model', 'questions'].includes(root)) return `field:${root}`
    }
  ...
  } catch { /* untrusted or non-JSON error bodies are not diagnostic text */ }
...
export function createJevClient(config) {
  ...
  const provider = jevProvider(baseUrl)
  const model = provider === 'vercel' ? JEV_VERCEL_MODEL : (String(config.model ?? '').trim() || JEV_DEFAULT_MODEL)
  ...
  const maxAskMs = config.maxAskMs ?? perRequestMs * (maxRetries + 1) + maxBackoffMs * maxRetries
  ...
  async function postOnce(body, signal, phase) {
    ...
    usage.httpAttempts++
    let res
    const requestSignal = withTimeouts(signal)
    try {
      ...
      const kind = requestSignal.aborted || name === 'TimeoutError' || name === 'AbortError' ? JEV_ERROR_KINDS.timeout : JEV_ERROR_KINDS.network
      throw new JevError(kind, { retryable: true, phase, attempts: usage.httpAttempts, detail: ['Error', 'TypeError', 'TimeoutError', 'AbortError', 'NetworkPolicyError'].includes(name) ? name : 'fetch_failed' })
    }
    if (res.status >= 300 && res.status < 400) {
      try { await res.body?.cancel() } catch { /* never retain a redirect body */ }
      const location = res.headers.get('location')
      let target = null
      try { target = location ? originOf(new URL(location, endpoint).toString()) : null } catch { /* malformed redirects are blocked too */ }
    ...
    if (!res.ok) {
      ...
      const detail = safeServerDetail(body, res.status)
      throw new JevError(kind, {
        status: res.status, retryable, phase, attempts: usage.httpAttempts,
        detail: detail?.includes(apiKey) ? `http_${res.status}` : detail,
    ...
    try {
      ...
      if (signal?.aborted) throw new JevError(JEV_ERROR_KINDS.cancelled, { phase, detail: 'aborted' })
      if (requestSignal.aborted) throw new JevError(JEV_ERROR_KINDS.timeout, { phase, retryable: true, detail: 'body_timeout' })
  ...
  // Vercel has an evaluation protocol, not TypeSafe's native /systemone or
  // OpenAI chat completions. Keep its wire format in the official SDK.
  async function postVercel(body, signal, phase) {
    const { evaluateVercel } = await import('./vercel.mjs')
    const requestSignal = withTimeouts(signal)
    let wireError = null
    try {
      const parsed = await evaluateVercel({ apiKey, state: body.state, questions: body.questions,
        signal: requestSignal, fetchImpl: async (url, init) => {
          try {
            const target = new URL(String(url))
            if (target.origin !== 'https://ai-gateway.vercel.sh' || target.username || target.password) {
              throw new JevError(JEV_ERROR_KINDS.redirectBlocked, { phase, detail: 'gateway_origin_mismatch' })
            }
            if (String(init?.body ?? '').length > maxRequestChars) throw new JevError(JEV_ERROR_KINDS.requestTooLarge, { phase })
            usage.httpAttempts++
            const response = await fetchImpl(url, { ...init, redirect: 'manual', signal: requestSignal })
            if (response.status >= 300 && response.status < 400) {
              try { await response.body?.cancel() } catch { /* close the credential-bearing redirect */ }
              throw new JevError(JEV_ERROR_KINDS.redirectBlocked, { phase, status: response.status })
            }
            if (!response.ok) {
              let text = ''
              try { text = await readLimited(response, 20_000) } catch { /* optional error body */ }
              const classification = classifyStatus(response.status)
              throw new JevError(classification.kind, { ...classification, phase, status: response.status,
                detail: safeServerDetail(text, response.status), retryAfter: retryAfterMs(response.headers, now()) })
            }
            const text = await readLimited(response, maxResponseBytes)
            if (/^\s*</.test(text)) throw new JevError(JEV_ERROR_KINDS.gatewayResponse, { phase, retryable: true, detail: 'html_body' })
            const { Response } = await import('undici')
            return new Response(text, { status: response.status, headers: response.headers })
          } catch (err) { wireError = err; throw err }
        } })
      return { parsed }
    } catch (err) {
      if (signal?.aborted) throw new JevError(JEV_ERROR_KINDS.cancelled, { phase, detail: 'aborted' })
      if (requestSignal.aborted) throw new JevError(JEV_ERROR_KINDS.timeout, { phase, retryable: true, detail: 'request_timeout' })
      if (wireError instanceof JevError) throw wireError
      // Never surface SDK error strings/request bodies: they can contain keys
      // and excerpts. A wire failure is retryable; invalid typed output isn't.
      throw new JevError(wireError ? JEV_ERROR_KINDS.network : JEV_ERROR_KINDS.malformedResponse,
        { phase, retryable: Boolean(wireError), detail: wireError ? 'gateway_network' : 'gateway_invalid_response' })
    }
  }
  ...
  return {
    ...
    describe() {
      return { endpointOrigin: expectedOrigin, model, provider, configured: true }
    },
    ...
    async ask(request) {
      ...
      const parents = [request.signal, config.signal].filter(Boolean)
      const parentSignal = parents.length > 1 ? AbortSignal.any(parents) : (parents[0] ?? null)
      const signal = parentSignal ? mergeSignals(parentSignal, maxAskMs) : AbortSignal.timeout(maxAskMs)
      const attemptsBefore = usage.httpAttempts
      throwIfAborted(signal)
      const started = now()
      let lastError = null
      for (let attempt = 0; attempt <= maxRetries; attempt++) {
        // A late abort during backoff must not start another HTTP request.
        if (signal.aborted) throw new JevError(parentSignal?.aborted ? JEV_ERROR_KINDS.cancelled : JEV_ERROR_KINDS.timeout, { phase, detail: 'aborted', attempts: usage.httpAttempts - attemptsBefore })
        try {
          const { parsed } = await (provider === 'vercel' ? postVercel : postOnce)(body, signal, phase)
          ...
          lastModel = /^[A-Za-z0-9_.:/-]{1,96}$/.test(reportedModel) && !reportedModel.includes(apiKey) ? reportedModel : model
          return {
            model: lastModel,
            entries: validated.entries,
            invalidIds: validated.invalidIds,
            unknownIds: validated.unknownIds.slice(0, 16).map((id) => String(id).replaceAll(apiKey, '[redacted]').slice(0, 96)),
            missingIds: validated.missingIds,
            shapeError: validated.shapeError,
            usage: reported,
            attempts: usage.httpAttempts - attemptsBefore,
            requestChars,
            tookMs: now() - started,
            phase,
          }
          ...
          error.attempts = usage.httpAttempts - attemptsBefore
          lastError = error
          if (signal.aborted) throw new JevError(parentSignal?.aborted ? JEV_ERROR_KINDS.cancelled : JEV_ERROR_KINDS.timeout, { phase, detail: 'aborted', attempts: usage.httpAttempts - attemptsBefore })
          const budgetLeft = attempt < maxRetries
          if (!error.retryable || !budgetLeft) throw error
          const wait = Math.max(error.retryAfter ?? 0, Math.min(maxBackoffMs, 300 * 2 ** attempt))
          if (wait >= maxAskMs - (now() - started)) {
            error.detail = 'retry_after_exceeds_budget'
            throw error
          }
          usage.retries++
          usage.backoffMs += wait
          try { await sleep(wait, signal) } catch {
            throw new JevError(parentSignal?.aborted ? JEV_ERROR_KINDS.cancelled : JEV_ERROR_KINDS.timeout,
              { phase, attempts: usage.httpAttempts - attemptsBefore, detail: 'backoff_aborted' })
          }
        }
      }
