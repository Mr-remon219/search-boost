...
export async function renderSkillFile(agentId, file) {
  // Rendered bundles are installed verbatim and compared byte-for-byte: normalise
  // line endings so a CRLF checkout (Windows defaults to core.autocrlf=true)
  // cannot change the output.
  const lf = (text) => String(text).replace(/\r\n/g, '\n')
  let body = lf(await readFile(file.source, 'utf8'))
  ...
  const tokens = {
    PARALLEL_HOST: body.includes('{{PARALLEL_HOST}}') ? lf(await readFile(parallelHostPath(agentId), 'utf8')).trim() : '',
