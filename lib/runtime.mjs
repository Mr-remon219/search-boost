...
import { describeJevForCapability, readJevConfig } from './jev-config.mjs'
import { createJevClient } from './jev/client.mjs'
import { runAdaptiveLoop } from './search/adaptive/loop.mjs'
import { selectEngineCandidates } from './search/adaptive/planning.js'
import { approvedResults, resultPages, validatePageInput } from './search/adaptive/pages.js'
import { ADAPTIVE_HOST_DEADLINES, ADAPTIVE_LIMITS, ADAPTIVE_THRESHOLDS } from './search/adaptive/limits.js'
...
export async function runFused({ query, queries, engineList, maxResults, includeDomains, excludeDomains, recency, complexity = 'medium', layer = null, enginePool, ranking = 'balanced', engineWeights, community = false, depth = null, minScore = 0, maxResultsCap = 10, candidateSelection = 'fused', signal }, { snapshot = runtimeSnapshot, xSearch = runXSearch } = {}) {
  ...
  if (!['fused', 'per_engine'].includes(candidateSelection)) throw new Error('Invalid candidate selection')
  ...
  const selected = candidateSelection === 'per_engine'
    ? selectEngineCandidates(rows.filter((row) => matchesDomains(row.url, includeDomains, excludeDomains)), route.engineNames, maxResults, { minScore })
    : selectDiverse(rows.filter((row) => matchesDomains(row.url, includeDomains, excludeDomains)), {
    limit: maxResults, minScore, includeDomains,
  })
...
const KEYED_OR_API_ENGINES = new Set(['tavily', 'brave', 'exa', 'anysearch'])
...
export function jevCapability() {
  return describeJevForCapability()
}

/**
 * adaptive_search — high-level, Jev-driven evidence loop shared by MCP / Pi / DSH.
 *
 * Jev selects engines, judges material and decides per-question coverage; this
 * core validates typed responses, owns every budget/threshold and executes only
 * whitelisted actions through the same runFused / runFetchPage paths the core
 * tools use. Jev is optional: without credentials the tool returns an honest
 * not_configured result without any network request.
 *
 * @param {{ questions: string[] }} input
 * @param {{ signal?: AbortSignal | null, audit?: { write: Function } | null, deadlineMs?: number, host?: 'mcp'|'pi'|'dsh', onProgress?: (message: string) => void }} [opts]
 */
export async function runAdaptiveSearch(input, opts = {}) {
  validatePageInput(input)
  opts.signal?.throwIfAborted()
  if (input.cursor !== undefined) return resultPages.read(input.cursor, input.page_size)
  const { page_size, ...searchInput } = input
  ...
  if (cfg.apiKey) {
    try {
      jev = createJevClient({
        ...
        maxRetries: ADAPTIVE_LIMITS.maxJevRetries,
        perRequestMs: ADAPTIVE_LIMITS.jevPerRequestMs,
        maxBackoffMs: ADAPTIVE_LIMITS.jevMaxBackoffMs,
        maxRequestChars: ADAPTIVE_LIMITS.maxRequestChars,
      })
    } catch {
      jev = null
    }
  }
  const deadlineMs = opts.deadlineMs ?? ADAPTIVE_HOST_DEADLINES[opts.host ?? 'mcp'] ?? ADAPTIVE_LIMITS.defaultDeadlineMs
  let completeEvidence = []
  const result = await runAdaptiveLoop(searchInput, {
    jev,
    signal: opts.signal ?? null,
    audit: opts.audit ?? NULL_AUDIT,
    deadlineMs,
    onProgress: opts.onProgress,
    limits: ADAPTIVE_LIMITS,
    thresholds: ADAPTIVE_THRESHOLDS,
    snapshot: runtimeSnapshot,
    runFused: (args) => runFused(args),
    runFetchPage: (url, focus, signal) => runFetchPage(url, focus, signal),
    onComplete: ({ evidence }) => { completeEvidence = evidence },
  })
  if (opts.diagnostics) return result
  const warnings = [...result.warnings]
  const dateExcluded = completeEvidence.filter((item) => item.status === 'date_unqualified')
  if (dateExcluded.length) warnings.push(`${dateExcluded.length} fragment(s) excluded: date unknown, outside the required window, or not established by the time-match assessment.`)
  const deferred = result.evidence.withText - completeEvidence.filter((item) => item.assessed).length
  if (deferred > 0) warnings.push(`${deferred} collected fragment(s) were not fully assessed and are absent from results.`)
  if (result.uncovered.length) warnings.push(`${result.uncovered.length} target(s) remain uncovered; returned URLs are useful evidence, not a complete answer.`)
  return resultPages.save(approvedResults(completeEvidence), { coverageComplete: result.questions.length > 0 && result.questions.every((q) => q.status === 'covered'), stopReason: result.stopReason, warnings }, page_size)
}
 ...
 * aborted once the last waiter leaves, and the entry lives until it settles.
 */
