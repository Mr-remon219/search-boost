...
export async function buildLegacyRelease({ version, outDir }) {
  ...
  const notice = '# search-boost-mcp — transition release\n\nThis legacy npm package now exposes **only `search-boost-mcp`**, not `search-boost`. The new `search-boost` npm package owns the `search-boost` command. Both packages can coexist without a shared global bin.\n\nUpdate the old package first, then install the new one:\n\n```sh\nnpm install -g search-boost-mcp@latest\nnpm install -g search-boost@latest\nsearch-boost\n```\n\nOne-command migration: `search-boost-mcp migrate -y` (or preview with `search-boost-mcp migrate --dry-run`). You can also run `node /path/to/search-boost-mcp/migrate.mjs -y` without relying on PATH. This installs/verifies the new package before migrating integrations, and stops if the future release is not yet available. Run `search-boost-mcp upgrade` to explicitly install the new package and migrate existing integrations. No forwarding plugin, postinstall migration, global legacy-package removal, or `--force` is used. Existing credentials/configuration are not deleted. Restart affected hosts after migration.\n\n'
