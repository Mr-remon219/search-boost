...
}
{
  ...
  assert('dsh registers all tools', ['fused_search', 'fetch_page', 'x_search', 'research_parallel', 'search_stats', 'adaptive_search'].every((n) => m.tools.has(n)))
  ...
  assert('dsh capability re-evaluates on every assembly', status.text().includes('default engine_pool: hybrid'))
  ...
  const emptySearch = await fused.execute({ query: 'fixture', engine_pool: 'api', ranking: 'research', engine_weights: { exa: 0 }, community: false }, {})
  assert('dsh passes new routing and returns effective diagnostics', emptySearch.enginePool === 'api' && emptySearch.ranking === 'research' && emptySearch.enginesUsed.length === 0 && emptySearch.communityUsed === false && emptySearch.warnings.length > 0)
  assert('dsh output schema declares every emitted top-level field', Object.keys(emptySearch).every((key) => key in fused.output.schema.properties))
...
// ---------------------------------------------------------------------------
{
  ...
  pi.default(mockPi)
  assert('pi registers all tools', ['fused_search', 'fetch_page', 'search-parallel-subagent', 'x_search', 'adaptive_search'].every((n) => tools.has(n)))
  ...
  assert('pi capability is dynamic on the next agent start', changed.systemPrompt.includes('default engine_pool: hybrid'))
  runtime.switchLayer('free')
  const notes = []
