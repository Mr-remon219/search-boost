...
// agents: claude malformed allow entries (non-string) must not crash doctor
{
  const home = mkdtempSync(join(tmpdir(), `search-boost-doctor-claude-malformed-${process.pid}-`))
  try {
    mkdirSync(join(home, '.claude'), { recursive: true })
    writeFileSync(
      join(home, '.claude.json'),
    ...
    writeFileSync(
      join(home, '.claude', 'settings.json'),
      `${JSON.stringify({ permissions: { allow: ['mcp__search-boost__*', null, 42] } })}\n`,
      'utf8',
    )
    const { report } = runDoctorInSubprocess(home, 'agents')
