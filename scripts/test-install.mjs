...
import {
  ...
  autoAllowAgentIds,
  CLAUDE_NATIVE_DENY_MARKER,
  CLAUDE_WEB_FETCH_DENY,
...
assert('claude native revert keeps allow', claudeOff.permissions.allow.includes('mcp__search-boost__*'))
assert('claude native revert removes owned deny', !claudeOff.permissions?.deny?.includes(CLAUDE_WEB_SEARCH_DENY))
...
// grok: marked block with START above [permission] is not legacy
const markedAbovePermission = `# SEARCH_BOOST_permission_START
[permission]
allow = [
  "MCPTool(search-boost__fused_search)",
]
...
`
assert('grok marked above permission not legacy', !hasLegacySearchBoostPermission(markedAbovePermission))
